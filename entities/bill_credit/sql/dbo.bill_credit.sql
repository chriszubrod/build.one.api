...
IF OBJECT_ID('dbo.BillCredit', 'U') IS NOT NULL
   AND NOT EXISTS (SELECT 1 FROM sys.columns
                   WHERE object_id = OBJECT_ID('dbo.BillCredit') AND name = 'CreatedByUserId')
BEGIN
    ALTER TABLE [dbo].[BillCredit] ADD [CreatedByUserId] BIGINT NOT NULL
        CONSTRAINT [DF_BillCredit_CreatedByUserId] DEFAULT (17);
END
GO
IF OBJECT_ID('dbo.BillCredit', 'U') IS NOT NULL
   AND NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = 'FK_BillCredit_CreatedByUser')
BEGIN
    ALTER TABLE [dbo].[BillCredit] ADD CONSTRAINT [FK_BillCredit_CreatedByUser]
...
CREATE OR ALTER PROCEDURE SetBillCreditQboIdentity
(
    @Id BIGINT,
