...
class ItemCostCodeConnector:
    ...
    def _stamp_cost_code_identity(
        ...
        `on_conflict` reuses `_record_duplicate_qbo_item_issue` (below) — the
    ...
    def _record_duplicate_qbo_item_issue(
        ...
        *,
        ...
        """
        Number-match duplicate (U-328/U-331). Reuses `duplicate_qbo_item` — the same
        DriftType `resolve_candidate`'s number-match guard already records.
        """
        details = (
