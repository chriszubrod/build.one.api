...
class ExpenseLineItemRepository:
    ...
    def read_by_qbo_identity(self, expense_id: int, qbo_id: str) -> Optional[ExpenseLineItem]:
        """
        Read an expense line item by its dbo-native QBO identity, scoped to its
        parent Expense (U-293b) — the line-level Phase-4 repoint seam, bypassing
        the qbo.PurchaseLine/qbo.PurchaseLineExpenseLineItem staging/mapping tables.

        Parent-scoped, not global: a QBO line id is unique only within its
        parent transaction, matching the live UQ_ExpenseLineItem_ExpenseId_QboId
        ...
        try:
            with get_connection() as conn:
                ...
                call_procedure(
                    cursor=cursor,
                    name="ReadExpenseLineItemByExpenseIdAndQboId",
                    params={"ExpenseId": expense_id, "QboId": qbo_id},
                )
                row = cursor.fetchone()
                return self._from_db(row)
        except Exception as error:
        ...
        *,
        ...
        try:
            with get_connection() as conn:
                ...
                call_procedure(
                    cursor=cursor,
                    name="SetExpenseLineItemQboIdentity",
                    params={
                        "Id": id,
                        "QboId": qbo_id,
                        "RealmId": realm_id,
                ...
                if row and getattr(row, "Stolen", False):
                    logger.warning(
                        "ExpenseLineItem %s stole QBO identity (qbo_id=%s realm_id=%s) from a "
                        "different ExpenseLineItem row under the same Expense — a stale duplicate "
                        "identity existed before this stamp",
                        id, qbo_id, realm_id,
                    )
        except Exception as error:
            logger.error(
                "Error stamping ExpenseLineItem QBO identity (expense_line_item_id=%s qbo_id=%s realm_id=%s): %s",
                id,
                qbo_id,
                realm_id,
                error,
