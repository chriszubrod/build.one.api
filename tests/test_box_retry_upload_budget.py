...
def test_for_uploads_budget_satisfies_invariant():
...
def test_for_uploads_retries_after_full_timeout_attempt(deterministic_retry):
    # Attempt 1 'consumes' a full 120s tier-C timeout then 503s; for_uploads()'s
    # large budget must still allow attempt 2, which succeeds.
    op = _FailThenSucceed(fail_times=1)
    result = execute_with_retry(op, RetryPolicy.for_uploads(), operation_name='box.excel.upload_version')
    assert result == {'ok': True}
    assert op.calls == 2


def test_for_writes_budget_exceeds_on_same_scenario(deterministic_retry):
    # The SAME slow-then-503 scenario under the old for_writes() 30s budget
    # budget-exceeds on attempt 1 -> raises with ZERO retries. This is the bug.
    # for_writes() allows multiple attempts by policy, so the single call proves
    # the BUDGET aborted the retry (not the attempt ceiling) -- the real branch.
    assert RetryPolicy.for_writes().max_attempts >= 2
    op = _FailThenSucceed(fail_times=1)
    with pytest.raises(BoxServiceUnavailableError):
        execute_with_retry(op, RetryPolicy.for_writes(), operation_name='box.excel.upload_version')
    assert op.calls == 1
