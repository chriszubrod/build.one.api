...
class BillExtractionAgent(Agent):
    ...
    async def run(self, context: AgentContext) -> AgentResult:
        ...
        try:
            ...
            project_match = self._match_project(
    ...
    def _match_project(self, project_hint: Optional[str], tenant_id: int) -> Optional[Dict]:
        """Match project by name or address hint."""
        if not project_hint:
            return None
        try:
            # Use existing fuzzy match on project name/address
            result = self.capabilities.entity.match_project(project_hint, tenant_id)
            if result.success and result.data:
                if isinstance(result.data, dict):
                    return result.data
                elif isinstance(result.data, list) and len(result.data) > 0:
                    # Take top match if confidence is high enough
                    top = result.data[0]
                    if hasattr(top, 'confidence') and top.confidence >= 0.7:
                        return {
                            "id": top.id,
