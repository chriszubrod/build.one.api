...
class QboPhysicalAddressClient:
    """
    Lightweight client for interacting with QBO CompanyInfo endpoint to retrieve PhysicalAddress.
    """
        ...
        self._client.headers.update(
            {
                ...
                "User-Agent": "build.one-qbo-physical-address-client/1.0",
            }
        )
    ...
    def get_company_info(self) -> QboCompanyInfoResponse:
        """
        Retrieve CompanyInfo from QuickBooks, which contains PhysicalAddress.
        """
        data = self._request("GET", "/companyinfo")
    ...
    def get_physical_address(self, qbo_id: Optional[str] = None) -> Optional[QboPhysicalAddress]:
        ...
        Args:
            qbo_id: Optional QBO ID (not used for CompanyInfo, but kept for API consistency)
        
        Returns:
            QboPhysicalAddress: The physical address from CompanyInfo, or None if not found
        """
