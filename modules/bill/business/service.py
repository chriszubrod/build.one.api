...
class BillService:
    ...
    def create(self, *, vendor_public_id: str, payment_term_public_id: Optional[str] = None, bill_date: str, due_date: str, bill_number: str, total_amount: Optional[Decimal] = None, memo: Optional[str] = None, is_draft: bool = True) -> Bill:
        """
        Create a new bill.
        """
    ...
    def update_by_public_id(self, public_id: str, bill) -> Optional[Bill]:
        ...
        # Resolve payment_term_public_id to payment_term_id
        if hasattr(bill, 'payment_term_public_id') and bill.payment_term_public_id:
            from modules.payment_term.business.service import PaymentTermService
            payment_term = PaymentTermService().read_by_public_id(public_id=bill.payment_term_public_id)
            existing.payment_term_id = payment_term.id if payment_term else None
        else:
            existing.payment_term_id = None
