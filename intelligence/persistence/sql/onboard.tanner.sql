-- Onboard Tanner Baker as a Project Manager user.
--
-- Project Manager role already exists (one of the 8 seeded human roles).
-- Idempotent — safe to re-run.
--
-- NOTE: UserProject rows carry RoleId = Project Manager so bill-submit
-- notifications correctly address Tanner as the project PM (per
-- project_review_notifications.md). Field Crew users leave RoleId NULL.
--
-- RUN:
--   .venv/bin/python scripts/run_sql.py intelligence/persistence/sql/onboard.tanner.sql

DECLARE @Now DATETIME2 = SYSUTCDATETIME();

DECLARE @Username NVARCHAR(100)   = 'bakertb';
DECLARE @PasswordHash NVARCHAR(255) =
    '$2b$12$YuJs44gQG456RjQ576pSJ.aPPFWsvAnfqF5P0XrNhbfXid63ueq5q';
DECLARE @Firstname NVARCHAR(50)   = 'Tanner';
DECLARE @Lastname  NVARCHAR(255)  = 'Baker';
DECLARE @Email     NVARCHAR(255)  = 'tanner@rogersbuild.com';
...
IF @RogersCompanyId IS NULL OR @RogersOrgId IS NULL OR @PMRoleId IS NULL
BEGIN
...
IF NOT EXISTS (SELECT 1 FROM dbo.UserRole WHERE UserId = @UserId AND RoleId = @PMRoleId)
BEGIN
    INSERT INTO dbo.UserRole (CreatedDatetime, ModifiedDatetime, UserId, RoleId, CompanyId)
    VALUES (@Now, @Now, @UserId, @PMRoleId, @RogersCompanyId);
    PRINT '  user-role link created (Project Manager)';
END
...
-- UserProject bulk WITH RoleId = Project Manager
-- (so bill-submit notifications address Tanner as PM)
...
SELECT @Now, @Now, @UserId, a.ProjectId, @PMRoleId
