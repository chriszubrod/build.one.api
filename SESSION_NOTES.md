...
**Remaining for Phase 2 close-out:** web UI minimum (Box status card + outbox/dead-letter visibility + mapping admin surface); App Insights alert config (dead_letter>0, oldest-pending>10m, mint failures); **prod deploy** (App Service settings BOX_CLIENT_ID/SECRET/ENTERPRISE_ID + ALLOW_BOX_WRITES=false initially; az acr build :latest + restart + verify tag ends :latest; func publish scheduler). Phase 3 (Excel) blocked on workbook copies from the client.
