...
**API email_specialist teardown** (`f14909ae`). Removed the agent package, its `app.py` registration, and the two agent-driving routes (`/admin/email/process_one`, `/admin/email/extract/{id}`). Kept `/admin/email/poll` + `/admin/email/recover_stuck` (no agent involved; now inert manual endpoints). Restored three doc items the WIP over-deleted: the review-submit notification runbook, its README index row, and its CLAUDE.md entry — `ReviewNotificationService` is still live and wired at `entities/review/business/service.py:198`. **Follow-up owed:** it forwards the original vendor email via `Bill.SourceEmailMessageId`, which new bills will no longer carry, so the feature is now dormant for new bills — decide whether to retire it properly.

**U-238c SQL applied to prod, in the documented order** — migration → `qbo.physical_address.sql` companion → the 8 `Set*QboIdentity` sprocs → reverse-lookup views. All 8 entities verified: `QboId` + `RealmId` columns, filtered unique index, and sproc present; `qbo.PhysicalAddress.RealmId` present; both views created.

- **Did NOT blanket-apply the 8 entity base files.** Diffing each file's sprocs against `sys.sql_modules` (comment-normalized, `CREATE OR ALTER`→`CREATE`) found **6 genuinely prod-drifted sprocs** unrelated to this unit — `CreateCustomer`, `DeleteCustomerById`, `CreateCostCode`, `CreatePaymentTerm`, `DeletePaymentTermById`, `DeleteAddressById`. A whole-file apply would have silently reverted them. Extracted only the 8 new sprocs instead (per U-238a's own TODO guidance) and re-verified afterward that all 6 remain drifted, i.e. un-clobbered. **These 6 are still unreconciled — base files are stale for those entities.**
- **Fixed a shipped defect that would have failed the apply** (`ba266543`): `SetAttachmentQboIdentity` had no `GO` before it in `dbo.attachment.sql`, so it was swallowed into `IncrementDownloadCount`'s batch — the documented T-SQL batch trap. SQL Server rejects `CREATE PROCEDURE` that isn't first in its batch. Only `dbo.attachment.sql` was affected; the other 7 were correct. Undetectable by pytest, which never executes these files.

**Deploy** (`ba266543`, ACR run `ca9d`, digest `sha256:bdbac530`, tagged `latest` + `ba266543`). Verified with a **sentinel**, not a bare health check: `/admin/email/process_one` → 404 (removed) while `/admin/email/poll` → 401 (retained). The first three probes returned 401 for both — the old container still serving — so a plain 200/401 check would have falsely confirmed at attempt 1.
...
**Context:** third sub-unit of the QBO staging-removal PROGRAM's Phase 2 (identity re-home), following U-238a's header shape (`00a3dec`) and U-238b's line-item shape. Scope: dbo-native `QboId`/`RealmId` on 8 reference entities (Vendor, Customer, CostCode, SubCostCode, PaymentTerm, Address, Attachment, BillCredit). **Backfill NOT yet run** — SQL migration + code shipped, dry-run only; deploy is SQL-first per U-238a/U-238b precedent, held on Chris's go-ahead.

**Design decisions vs assignment brief (empirical corrections):**
- **BillCredit has no SyncToken** — verified zero live push path (`sync_to_qbo`/`push_to_qbo` grep under `vendorcredit/` and `bill_credit/`); pull-only like Project/Company in U-238a. All 8 reference entities are realm-scoped `(QboId, RealmId)` unique indexes; none are per-parent line-item scoped.
- **Fan-out reverse-lookup has no DB-level cross-table uniqueness** — QBO Item ids route to CostCode OR SubCostCode via `parent_ref`; Customer ids route to Customer OR Project via `job`. SQL Server cannot enforce cross-table uniqueness, so Phase 4 contract is two read-only views (`dbo.vw_QboCustomerIdentity`, `dbo.vw_QboItemIdentity`) plus explicit fan-out overlap checks in backfill + drift scripts (ERROR if intersection non-empty).
- **Attachment 1:1 design note** — `qbo.AttachableAttachment` is strict 1:1 both directions, but QBO mints one Attachable per (file, entity) pair; the same PDF on two Bills yields two Attachables and the second can never bind a mapping (by design, not U-226 dangling). Backfill/drift output labels `unmapped_staging` distinctly from `dangling` mappings.
- **PhysicalAddress RealmId plumbing gap (fixed in-unit)** — `qbo.PhysicalAddress` had no `RealmId`; vendor/customer `_upsert_physical_address()` received `realm_id` but dropped it on create/update. Fixed: column + sproc threading + dual-write. Address backfill adds **Stage 0** to populate historical NULL `qbo.PhysicalAddress.RealmId` via parent-id suffix parse (`_bill`/`_ship`) + `qbo.Vendor`/`qbo.Customer` lookup before dbo stamp pass.
- **PhysicalAddress composite QboId collision (discovered, out of scope)** — Vendor id N and Customer id N synthesize the same composite QboId (`N_bill`); `ReadQboPhysicalAddressByQboId` can collide across entity types. Not fixed here — see Follow-ups.

**Built (mirrors U-238a/b):** `scripts/migrations/238c_qbo_identity_reference.sql`; 8 `Set*QboIdentity` sprocs (entity base SQL files, steal-then-claim + no-op ROWVERSION guard, no SyncToken); 8 `repo.set_qbo_identity()` methods; `REFERENCE_ENTITY_SPECS` in `integrations/intuit/qbo/base/identity_drift.py`; reverse-lookup views in `integrations/intuit/qbo/base/sql/qbo.identity_reverse_lookup_views.sql`; fan-out overlap helper `integrations/intuit/qbo/base/identity_fanout.py`; dual-write at all mapping-create/update/heal call sites (stamp-before-mapping-create, propagate stamp failure — header ordering not line-item log-and-continue); `qbo.PhysicalAddress.RealmId` staging fix; `scripts/backfill_qbo_identity_reference.py` (per-row verification, zero-collision assertion, Address Stage 0, fan-out checks, dangling exclusion); `scripts/check_qbo_identity_drift_reference.py`; `scripts/check_qbo_identity_drift_all.py` orchestrator (17 topologies); headers/lines drift scripts now return exit codes for aggregation.

**Pass 1 (Codex + 5-lens Workflow hunt in parallel, 26 agents, refute-by-default; 7/7 confirmed, 0 refuted/unadjudicated) + a bounded 2nd fix round:**
- Round 1: Codex (`gpt-5.5`, xhigh) and the Workflow hunt's `sproc`/`dualwrite`/`scope_and_tests` lenses **independently triangulated on the same P1** (4 separate sources, no shared context): `sync_from_qbo_to_address`'s trailing identity-stamp block stamped `dbo.Address` with the QBO identity of the *currently-syncing* `QboPhysicalAddress`, even in the "cannot remap" branch where a street/city match had already resolved `mapping` to a **different** `QboPhysicalAddress` (common case — a vendor's bill/ship addresses often share street+city). Concrete failure: syncing the second address of a pair silently overwrote the first Address's stamped identity, so `dbo.Address.QboId` permanently disagreed with its own `qbo.PhysicalAddressAddress` mapping row and oscillated on every re-pull — the drift detector this same unit ships would have reported real, connector-caused corruption as routine "drift." Fixed with a guard requiring `mapping is not None and mapping.qbo_physical_address_id == qbo_physical_address_id` before stamping; regression test added. Also confirmed and fixed: a migration/base-file deploy-coordination gap (`qbo.PhysicalAddress.RealmId` lives in `qbo.physical_address.sql`, not the numbered migration — SESSION_NOTES deploy order now states it as a **required** companion step, plus a fail-fast preflight column check added to the backfill script); Stage 0's parent-realm lookup missed multi-realm ambiguity within a single table (fixed to union `DISTINCT RealmId` across **both** `qbo.Vendor` and `qbo.Customer`, not per-table `fetchone()`); the backfill's per-row verification was row-count-delta based and could false-pass when pre-existing orphan-stamped rows masked still-NULL mapped rows (rewritten to be mapping-anchored); backfill assertions (mismatch/collision/fan-out) were logged but never affected the process exit code (fixed: `main() -> int`, `raise SystemExit(main())`, matching the sibling drift scripts' already-established pattern); plus test-coverage gaps (dual-write ordering only tested for 2/8 connectors, the new `identity_fanout.py` module had zero tests, `REFERENCE_ENTITY_SPECS` topology only fully pinned 1/8 entries) — all closed.
- Round 2 (bounded, Codex only, scoped to the fix diff): **PASS, no findings.** Independently re-traced all 4 targeted fixes against the real code and re-ran the targeted suite (47 green) itself before verdict.

**Pass 2 (`/simplify`, 4 agents in parallel):** reuse, simplification, and altitude lenses **independently converged on the same finding**: `integrations/intuit/qbo/base/identity_fanout.py`'s `check_all_fanout_overlaps()` aggregator was dead code in production — both `backfill_qbo_identity_reference.py` and `check_qbo_identity_drift_reference.py` reimplemented its body inline instead of calling it (3 copies of the same 4-line "run both, combine, print" logic reduced to 1). Altitude also flagged that `HeaderEntitySpec` (the shared dataclass this unit reuses for `REFERENCE_ENTITY_SPECS`) was now a misleading name and its docstrings factually wrong (still described "5 entities... headers"); renamed to `FlatEntitySpec` across all 5 producer/consumer files (~24 occurrences: `identity_drift.py`, both header scripts, both reference scripts) — verified zero test churn (no test references the class name directly) and zero stray references post-rename via repo-wide grep. Efficiency lens found 2 genuinely new (not pre-existing/accepted) issues: Attachment's "blob healthy" cache-hit path had turned a pure read into an unconditional identity-stamp write on every realm pull for the steady-state-majority case (fixed to skip the stamp when the already-fetched row's values already match, in-memory, no extra query); Address Stage 0's parent-realm lookup issued 2 SELECT queries **per row** — an N+1 across a backfill expected to process thousands of historical rows (fixed to precompute two `qbo_id -> set(realm_id)` dicts once before the loop, same matched/unmatched/ambiguous semantics, O(1) per row after). Three more trivial fixes applied: a redundant duplicate `_assert_physical_address_realm_id_column` call removed from `backfill_entity()`, a stale/misplaced `# Create mapping` comment fixed in SubCostCode's connector (copy-paste artifact vs. the other 7 connectors), `_verify_entity`'s unused 2-tuple return collapsed to a single bool. **Deliberately skipped, with reasoning (not silently dropped):** the connection-per-`set_qbo_identity()`-call pattern (already tracked as a TODO.md follow-up from U-238a; confirmed present but not newly worsened by this unit's 8 call sites); `check_qbo_identity_drift_reference.py`'s per-row Python-side classification (mirrors the already-logged, already-deferred header-drift-script inefficiency — fixing only the reference twin would leave 3 scripts inconsistent, the same reasoning U-238b used to skip the identical case for its own line-drift script); `identity_fanout.py`'s hardcoded-per-pair shape rather than a data-driven registry (a reasonable future improvement, but no 3rd fan-out pair exists or is planned — not urgent).

**Verify:** **1822 pytest green** in a CLEAN WORKTREE (`git worktree add` from HEAD `6d1b08a`, this unit's full diff applied via `git apply`, `.env` copied in, `.venv` from the main tree, worktree deleted after — exit code 0, all green, no failures/errors). Targeted identity-test modules (`test_qbo_identity_reference.py` + `test_qbo_identity_fanout.py` + `test_update_read_column_parity.py`) = 120 green standalone. `py_compile` clean on every changed file. No API surface added — no `/docs` refresh owed (backend-only schema/connector change, matches U-238a/U-238b precedent).

**Follow-ups (deferred):**
- **PhysicalAddress composite QboId collision** — Vendor id N and Customer id N can synthesize identical composite ids (`N_bill`/`N_ship`); no type discriminator in composite string; `ReadQboPhysicalAddressByQboId` collides. Needs composite redesign or type prefix, separate unit.
- **`sync_from_address_to_qbo` reverse push** — no dbo identity stamp added; path pushes local Address fields to staging only; dbo Address already carries QboId when reachable via existing mapping. Re-evaluate if reverse push becomes load-bearing for identity.
- **`set_qbo_identity()` per-call connection cost, now at 21 call sites total (U-238a headers + U-238b lines + U-238c reference)** — escalating priority again per Pass 2's efficiency lens; same TODO.md-tracked pattern, unfixed across all three sub-units for consistency; a future unit should thread an optional `cursor` param through all three generations at once.
- **`check_qbo_identity_drift_reference.py` pulls every row into Python for classification** — third copy of the same already-logged header-drift-script inefficiency (`check_qbo_identity_drift_headers.py`, `check_qbo_identity_drift_lines.py`); a future fix should cover all three scripts together, not piecemeal.
- **`identity_fanout.py` hardcoded per-pair, no data-driven registry** — fine at today's 2 known fan-out pairs; genericize (mirroring `identity_drift.py`'s spec-tuple pattern) only if/when a 3rd fan-out pair appears.
- **Deploy ordering (SQL-first, single cutover step — do not split):**
  1. `scripts/migrations/238c_qbo_identity_reference.sql` — dbo columns + filtered unique indexes on the 8 reference entities only.
  2. **REQUIRED companion (not optional):** `integrations/intuit/qbo/physical_address/sql/qbo.physical_address.sql` — adds `qbo.PhysicalAddress.RealmId` + threads it through PhysicalAddress sprocs. The numbered migration deliberately does **not** carry this staging-table DDL; skipping this file leaves Address Stage 0 and dual-write failing at runtime.
  3. Eight dbo entity base SQL files (Vendor, Customer, CostCode, SubCostCode, PaymentTerm, Address, Attachment, BillCredit) — each owns its `Set*QboIdentity` sproc.
  4. `integrations/intuit/qbo/base/sql/qbo.identity_reverse_lookup_views.sql` — fan-out reverse-lookup views.
  5. Deploy dual-write code, **then** run `scripts/backfill_qbo_identity_reference.py --apply` (never backfill before dual-write).

## Session: U-238b — Staging-removal Phase 2, line items (2026-08-14)

**Context:** second sub-unit of the QBO staging-removal PROGRAM's Phase 2 (identity re-home), following U-238a's header-entity shape (`00a3dec`). Scope: dbo-native `QboId`/`RealmId` on 4 line-item tables (BillLineItem, InvoiceLineItem, ExpenseLineItem, BillCreditLineItem), 66,265 mapping rows total. **Backfill NOT yet run** — SQL migration + code shipped, dry-run only; deploy is SQL-first per U-238a's own precedent, held on Chris's go-ahead.

**Map answered the assignment's central design question empirically, not theoretically.** QBO line ids are NOT unique per-realm (unlike header ids) — verified against prod (read-only): 42 same-realm collision groups in `qbo.BillLine`, 274 in `qbo.InvoiceLine`, 6 in `qbo.PurchaseLine`, 2 in `qbo.VendorCreditLine`, all across *different* parent transactions. A header-style `(QboId, RealmId)` global unique index would have hard-failed the backfill partway through. Correct key is **per-parent**: `(BillId, QboId)` / `(InvoiceId, QboId)` / `(ExpenseId, QboId)` / `(BillCreditId, QboId)` — verified **zero** collisions under this scope across all 4 entities' actual eligible rows before writing any code. No `SyncToken` at line level (QBO SyncTokens are per-transaction, already on the header — confirmed structurally, not just by inference). Re-stamp (not null-on-regeneration) is the right semantic: all 4 line connectors already share one shape — create branch, update/re-pull branch, and a separate stale-line-cleanup path that deletes the old mapping+staging row on QBO id regeneration and lets the existing fingerprint-readopt fall through into the *same* update branch as an ordinary re-pull, so 2 dual-write sites (create + update) per connector are sufficient — no third site needed for "regeneration."

**BillCreditLineItem judgment call:** the BOARD's U-238 PROGRAM note said BillCredit header+line identity should wait for U-225 (unresolved `qbo.VendorCreditBillCredit` CASCADE-vs-NO_ACTION FK divergence), and U-238a excluded BillCredit's header entirely for that reason. Traced U-225 and confirmed it's isolated to that *header* mapping table — `qbo.VendorCreditLineItemBillCreditLineItem` (the line mapping table) has clean, undivided CASCADE FKs, structurally identical create/update/cleanup shape to the other three connectors. Included BillCreditLineItem in scope on that basis; flagged explicitly at Gate 1, Chris approved.

**Built (mirrors U-238a):** `scripts/migrations/238b_qbo_identity_lines.sql` (additive, `NOT EXISTS`-guarded, per-parent filtered unique indexes); 4 new `Set<Entity>LineItemQboIdentity` sprocs (one per entity's base SQL file) — same CASE-WHEN/no-op-guard/steal shape as U-238a's header sprocs, but the "steal identity from a stale duplicate" self-join is scoped to siblings under the *same parent* (not global); 4 new `repo.set_qbo_identity()` methods; dual-write at 8 call sites (create+update × 4 connectors); `scripts/backfill_qbo_identity_lines.py` (batched, dry-run-by-default, **per-row value verification** — not just counts — plus a zero-collision assertion, mirroring what the assignment demanded after U-238a's count-only gap); `scripts/check_qbo_identity_drift_lines.py` (twin of the header drift detector, extra staging-header JOIN hop since line staging tables don't carry their own RealmId); `LineEntitySpec`/`LINE_ENTITY_SPECS` added to `integrations/intuit/qbo/base/identity_drift.py` alongside the existing header specs.

**Pass 1 (Workflow, 2 rounds, 45+23 agents, refute-by-default; 0 dead/errored, 0 unadjudicated):**
- Round 1 (3 lenses over the full diff): **10 confirmed findings**, all fixed. The headline one, found independently by all 3 lenses (9/9 verify votes): the Purchase/Expense connector's new identity-stamp call sat inside the same try/except as `create_mapping()`, whose broad `except Exception` deletes the just-created `ExpenseLineItem` on ANY failure — so a transient stamp failure (after the mapping already committed) manufactured a fresh **dangling mapping row and lost real data**, exactly the failure class this unit exists to measure, not create. A second finding, also new: the per-parent "steal" self-join in all 4 sprocs required the sibling's `RealmId` to match too, but the unique index is `(ParentId, QboId)` only — a mismatched/NULL RealmId (reachable via a third finding, Invoice's uncached per-line N+1 realm_id lookup) could let a real conflict escape the steal and hard-fail the final UPDATE on the unique constraint instead of self-healing. Also fixed: the Stolen-identity audit signal was silently lost when the steal fired but the target row itself needed no change (OUTPUT-gated on a no-op guard); zero test coverage on the connector that actually had the bug; a misleading VendorCredit log message.
- Round 2 (re-hunt scoped to the fix diff, 2 lenses): caught that round 1's core principle — "a stamp failure must never be conflated with a mapping-creation failure, and must never abort otherwise-successful work" — was only generalized to Purchase's CREATE path, not to Bill's CREATE path (**P1, worse blast radius: uncaught, triggers `rollback_orphan_header` on the NEW-Bill path, deleting an already-correctly-synced Bill+BillLineItem+mapping**), Invoice's CREATE path (P2, milder — swallowed), or any of the 4 connectors' UPDATE branches (P2 — the common steady-state re-pull case; an unguarded stamp failure there could propagate into a `RuntimeError` and hold the pull watermark over a purely cosmetic identity-cache write). All fixed — every one of the 8 dual-write call sites now has the identical narrow "log and continue, never propagate" shape. Also caught and fixed: missing test coverage (VendorCredit's UPDATE path, Invoice's header-connector-level realm_id threading) and cosmetic whitespace scope-creep in `dbo.invoice_line_item.sql` (self-corrected after a first attempt over-removed blank lines instead of restoring the original single line — verified the file returns to a clean append-only diff against HEAD).

**Codex unavailable a third time this wave** (workspace out of credits, confirmed via direct `codex exec` invocation, not assumed). Per this wave's standing rule, did not ship with a caveat — raised the mutation bar instead: hermetic mutation tests (± `__pycache__` clearing, confirmed green-on-unmutated first) on both P1 fixes (Purchase's and Bill's create-path try/except restructuring) — reverted each fix in place, confirmed the regression test fails exactly as expected, restored, confirmed green again. Both 1/1 caught.

**Pass 2 (`/simplify`, 4 agents in parallel):** 3/4 angles independently converged on the same primary finding — the "try stamp, log-and-warn on failure" shape, now proven correct through 2 fix rounds, was duplicated verbatim 8 times across the 4 connector files. Extracted into one shared free function, `stamp_line_identity_or_warn()`, in `integrations/intuit/qbo/base/identity_drift.py` (composition via a free function — matches this codebase's established idiom for cross-connector QBO concerns, e.g. `field_ownership.py`, `reconciliation_recorder.py` — not inheritance; verified this doesn't collide with the U-238a TODO's P3 note, which is about a *different* question, enforcing call-site *coverage*, not deduplicating the call *body*). Also fixed: `backfill_qbo_identity_lines.py`'s `_stamp_via_sproc` was hand-rolling positional-param `cursor.execute` instead of reusing `shared.database.call_procedure` (the exact helper every repo's `set_qbo_identity` already uses) — switched to named params, matching convention; removed a pointless `ENTITY_SPECS = LINE_ENTITY_SPECS` alias in the drift script. **Deliberately skipped, with reasoning, not silently dropped:** (a) two efficiency findings — per-line dual-write multiplies DB-connection-open cost from bounded-by-header (U-238a) to unbounded-by-line-count, and the drift script pulls every touched row into Python instead of aggregating in SQL — both are this unit *correctly mirroring* an already-known, already-deferred inefficiency in the HEADER twin scripts/connectors (U-238a's own TODO.md follow-ups say the same thing about `set_qbo_identity()`'s per-call connection and `check_qbo_identity_drift_headers.py`'s Python-side classification); fixing only the line side now would leave the two twins inconsistent, and the real fix belongs in a future unit touching both. (b) two reuse findings that the backfill/drift *scripts* duplicate ~45-75 lines of control-flow skeleton against their header siblings — real, but restructuring two already-shipped, already-tested script pairs for shared scaffolding is bigger surgery than a behavior-preserving Pass 2 should risk; noted as a future consolidation candidate.

**Verify:** **1756 pytest green** (1755 baseline post-fix-rounds + 1 new Bill create-path regression test added directly during Pass 1 verification, mirroring the one Composer added for Purchase). `py_compile` clean on every changed file. No API surface added — no `/docs` refresh owed (confirmed: backend-only schema/connector change, matches U-238a's precedent).

**Owed, not done here (deferred, tracked here since `TODO.md` carries unrelated foreign WIP this session deliberately avoided touching — see below):**
- Escalate priority on U-238a's existing TODO.md follow-up ("`set_qbo_identity()` opens its own connection on every call") — this unit's Pass 2 efficiency lens re-confirmed the same pattern now compounds at line-item scale (unbounded by line count per pull, not just +1 per header).
- Same for U-238a's "`check_qbo_identity_drift_headers.py` pulls every row into Python" follow-up — `check_qbo_identity_drift_lines.py` mirrors it; a future fix should cover both scripts together.
- Future consolidation candidate: `backfill_qbo_identity_lines.py`/`check_qbo_identity_drift_lines.py` duplicate meaningful control-flow skeleton against their header-twin scripts (Pass 2 reuse-angle finding, not fixed — see above).
- **`TODO.md` was not touched this session** — it carries an unrelated, already-dirty entry from a different session (an OL-14 incident note) that predates this session; per this session's own Gate-1 scoping rule (never sweep foreign WIP), the 3 items above should be appended to `TODO.md` in a small separate, clean commit once that file is safe to touch again.
- Deploy ordering, unchanged from U-238a's lesson: SQL-first (migration + 4 entity base files, diffed against `sys.sql_modules` comment-normalized first), dual-write deployed BEFORE the backfill script runs (never the reverse — backfill-first would silently miss anything changing in QBO between the two).

## Session: U-228 — Bound the unbounded QBO watermark hold (2026-08-12)

**Context:** booked as a latent VendorCredit risk quantified by U-218a's Pass 2 efficiency lens (unswallowing the VC line connector activated a hold-on-failure that never bounds). Went live mid-assignment: as of 2026-08-12 20:25 UTC the `bill` watermark had been held at 19:15:00 for 90+ minutes on 11 Walker Lumber "Service Charge" bills (qbo_ids 75071-75083, all `AccountBasedExpenseLineDetail`, no ItemRef/CustomerRef), with zero `qbo.ReconciliationIssue` rows recorded — the failure was invisible outside App Insights.

**Map found the connector has no bug.** Static tracing through `BillBillConnector.sync_from_qbo_bill` / `BillLineItemConnector.sync_from_qbo_bill_line` for these 11 records turned up nothing — every branch looked clean. Rather than trust that, ran the *actual* connector against the *actual* live prod rows: a single pyodbc connection opened directly (bypassing `shared/database.py`'s per-call auto-commit), every repo call in the real call chain executed for real, then an explicit `conn.rollback()` + close at the end — zero lasting writes, fully verified via a second read-only pass. With `system_authz()` correctly set (matching what `_require_drain_secret` sets for the real admin endpoint), all 11 bills projected cleanly: Bill + BillLineItem + both mappings, no exception. (A first pass *without* setting authz context did reproduce a "not accessible to the current actor" exception — but that was purely a test-harness artifact of skipping the auth setup, not a real prod path; re-run with proper context, it vanished. Worth recording since it looked like a strong lead for a few minutes.)

Root cause is almost certainly the pre-existing, already-documented P0 (TODO.md, "invoice-sync duplication ratchet", 2026-08-07): `POST /api/v1/admin/sync/qbo/{entity}` has no `sp_getapplock`, and the scheduler's client-side `REQUEST_TIMEOUT_SECONDS=60` doesn't cancel the server-side `asyncio.to_thread` work when the client gives up — so a bill-pull run re-processing an ever-growing held window (a self-reinforcing spiral: the longer the hold, the bigger the window, the longer the run) can still be executing when the *next* 15-min tick fires. Two overlapping runs racing to `create_mapping()` for the same qbo_id would hit real `UNIQUE` constraints on both `qbo.BillBill` and `qbo.BillLineItemBillLine` — confirmed both indexes exist. **Deliberately not fixed in this unit** — a concurrency-control change to a live write-path admin endpoint is a materially different, riskier change than bounding a hold, and deserves its own adversarial pass (same reasoning that split U-221 out of U-220). Booked as TODO.md follow-up (a), tied to the existing P0 rather than a fresh entry.

**Built:** `WatermarkRun.commit()` (`scripts/sync_helper.py`) bounds a hold **time-based**, not tick-counted — reuses `Sync.ModifiedDatetime` (falling back to `CreatedDatetime`) as the anchor, which already freezes at exactly the last successful advance since a hold-commit was always a pure no-op write; no schema change. Anchored against `WatermarkRun.query_start` (already captured once at `open()`), not a fresh `datetime.now()` — this also happens to be exactly the injection point the test suite already knew how to control. `QBO_WATERMARK_HOLD_BOUND_SECONDS` defaults to 7200 (2h), sized against the same day's real data: purchase held ~47 minutes and self-cleared, so the default gives genuine transients ample room while bounding worst-case silent-loss exposure. Below the bound: **byte-identical** to today (pure hold, zero writes). At/past it: writes one **critical** `qbo.ReconciliationIssue` per blocking id via the existing `base/reconciliation_recorder.py::record_mapping_issue`, then force-advances exactly like a clean success (respects `end_date`). `staging_failed_ids` (genuinely real QBO ids, per `SyncOutcome`'s own staging-loop provenance) are recorded at face value; `projection_failed_ids` (internal `qbo.<Entity>` staging PKs — the pre-existing `qbo.* Id ≠ dbo.* Id` hazard class) are resolved back to the real QBO id via a new `_QBO_SYNC_ENTITY_META` registry's `read_by_id(id).qbo_id` (10 of 11 entities; `reimburse_charge`'s repo has no `read_by_id`, and any lookup failure, fall back to a details string that says so honestly instead of mislabeling a staging PK as a QBO id).

Separately, `BillBillConnector.sync_from_qbo_bill`'s header `create_mapping()` guard widened `except ValueError` to also catch `DatabaseConstraintError` — a sibling of `ValueError`, not a subclass, that a concurrent-race `INSERT` on `qbo.BillBill`'s `UNIQUE` constraints can raise, and which previously escaped both the narrow except *and* the `rollback_orphan_header` safety net (wired only around `_sync_line_items`). The identical widening was tried on `BillLineItemConnector`'s line-level `create_mapping()` too, then **reverted** — see Pass 1.

**Pass 1, hunt one (Workflow, 3 lenses, 21 agents, refute-by-default; 0 dead/errored):** 5 confirmed, 1 correctly refuted.
- **P1, confirmed 3/3 — the line-connector widening was a live regression.** `dbo.BillLineItem` carries *no* uniqueness constraint of any kind (unlike `dbo.Bill`, protected by `UQ_Bill_VendorId_BillNumber_BillDate`), so silently swallowing a `DatabaseConstraintError` there converts a self-healing failure (rollback on the new-bill CREATE path via `rollback_orphan_header`, watermark hold on the existing-bill UPDATE path) into a **permanent, undetectable duplicate** `BillLineItem` — no reconciliation detector walks local-line→mapping in that direction. Reverted to `except ValueError` only, comment recording why, mutation-tested regression test added (`tests/test_qbo_bill_line_item_mapping_exceptions.py`) pinning that a `DatabaseConstraintError` there must propagate.
- **REFUTED, 0/3 confirmed — the header-connector mirror finding.** Three independent skeptics traced the identical race through `dbo.Bill` and found the loser fails at `bill_service.create()` itself (blocked by `UQ_Bill_VendorId_BillNumber_BillDate` and/or the app-level duplicate pre-check), *before* ever reaching the widened `create_mapping()` except clause — the header widening is real but structurally inert for this race, so it was kept.
- **P1, confirmed 3/3 — id-keyspace bug.** `_record_bound_forced_advance` originally wrote `projection_failed_ids` (staging PKs) into `qbo.ReconciliationIssue.QboId` as if they were real QBO ids, alongside `staging_failed_ids` (which genuinely are) — for the more common projection-failure case, the durable trail the whole unit exists to create would have been untraceable back to the real QBO transaction. Fixed by splitting the two lists and (after the fix-round hunt below) building the `read_by_id().qbo_id` resolver rather than leaving it permanently unresolved.
- **P2, confirmed 3/3 — zero test coverage** on the entire new bound-exceeded branch. **P3, confirmed 3/3 — cosmetic:** `entity_type` was written lowercase (`self.entity`, e.g. `"bill"`), inconsistent with every other `qbo.ReconciliationIssue` writer's capitalized convention.

One workflow-authoring bug on my own side: the aggregation script mis-nested a `pipeline()` call (an inner `pipeline()`'s array result replaced what should have been a `{lens, findings, dead}` object), crashing the post-processing JS *after* all 21 agents had already completed successfully — 0 errored, 0 empty. Caught by the failed task-notification (which correctly reports agent-level success separately from script-level failure), fixed the aggregation, and resumed via `resumeFromRunId` — 100% cache hit on all 21 agents, zero re-spend.

**Fix round, re-hunted (2 lenses, 14 agents, scoped to the fix diff):** 4 more confirmed, all P2/P3. Still-zero-coverage on the *new* id-split/resolver logic (closed with 13 new tests, including boundary-exact and realm-resolution-failure cases). A log line (`qbo.sync.watermark.bound_forced_advance`) that, three lines below the comment explaining the id-keyspace fix, still concatenated both lists under one `blocking_qbo_ids=` name — split into two named fields. A one-character comment typo (`len("reimburse_charge")` is 16, not 17). A genuine "missed opportunity, not a bug": the fix's own comment claimed no cheap resolution existed for `projection_failed_ids`, but 10 of 11 staging repos already expose `read_by_id(id).qbo_id` — built out into the registry rather than left as a TODO. Two verify agents independently reported an anomalous file-write code block inside `sync_helper.py` during one read — re-verified directly (byte count, md5, grep for `scratchpad`/`exec`/`eval`/`subprocess`/`open(`) and confirmed **not present** in the real file; a transient model artifact, not a real prompt injection, and the agents correctly refused to act on it regardless.

**Hermetic mutation matrix, 4/4 caught** (`__pycache__` cleared between every mutation; confirmed green-on-unmutated first, byte-identical restore confirmed after each). `and`→`or` (the exact U-217-class inverted-safe-default shape) — caught. Whole-block deletion — caught. Neutered `_record_bound_forced_advance` (never records an issue) — caught. Off-by-one (`held >= bound` → `held > bound`) — **initially survived**: every existing test exercised a hold orders of magnitude past the bound, none at the exact boundary. Closed by adding `held == bound` and `held == bound − 1s` tests, then re-verified caught.

**Pass 2 (4 angles, applied):** merged `_QBO_SYNC_ENTITY_TYPE_LABELS` + `_QBO_STAGING_REPO_MODULES` (two parallel dicts keyed on the same 11 entity strings, with nothing enforcing they stay in lockstep) into one `_QBO_SYNC_ENTITY_META` registry — two independent lenses (simplification + altitude) converged on this, altitude citing `integrations/intuit/qbo/base/field_ownership.py`'s `_REGISTRY` pattern as the established precedent for exactly this shape of per-entity QBO metadata. Switched the registry's staging-repo field from a stringly-typed `importlib.import_module(module_path)` dispatch to plain static top-of-file imports of the 10 repo classes — verified empirically (not just by reading) that this introduces no circular import. Reused `integrations/sync/persistence/repo.py::_parse_sync_last_sync` instead of a self-acknowledged duplicate (`_parse_sync_datetime`'s own docstring said "mirror" — literal admission of the violation). Converted a test helper's 4-item manual `unittest.mock.patch` list (`with patchers[0], patchers[1], patchers[2], patchers[3]:`, repeated at 4 call sites) into a proper `@contextlib.contextmanager`. Collapsed a duplicated 7-keyword `record_mapping_issue(...)` call shape (staging loop vs. projection loop, differing only in `qbo_id`/`details`) into one closure. **Declined:** factoring `_watermark_hold_bound_seconds`/`_watermark_overlap_seconds`'s near-identical env-int-parsing shape into a shared helper — only 2 call sites, both already in the same file, rule-of-three not met.

**Verify:** **1620 pytest green** (1605 baseline, **+15** — `tests/test_qbo_watermark_runner.py` +13 incl. the boundary-exact and realm-failure cases, `tests/test_qbo_reconciliation_recorder.py`'s AST guard extended and its entity list now *derived* from the real registry instead of hand-typed, new `tests/test_qbo_bill_line_item_mapping_exceptions.py` +2), `py_compile` clean, no circular import from the Pass-2 static-import switch (confirmed via a direct `import sync_helper` sanity check listing all 11 registry keys). **No SQL step** — reuses existing `dbo.Sync` columns. Container-only deploy. Codex was out of credits for the independent-reviewer leg (same as U-218e) — relied on the two Workflow hunts instead.

**Owed, booked in TODO.md, not done here:** (a) the missing-`sp_getapplock` root cause, tied to the existing 2026-08-07 P0 rather than a fresh unit; (b) the same `except ValueError` → also catch `DatabaseConstraintError` widening, evaluated per-table for a uniqueness constraint first, across ~13 other `create_mapping()` call sites in the QBO connector fleet; (c) `CLAUDE.md` needs `QBO_WATERMARK_HOLD_BOUND_SECONDS` in its env-var list — blocked by the same parallel-session foreign WIP as U-217/U-220; (d) a per-entity `QBO_WATERMARK_HOLD_BOUND_SECONDS_<ENTITY>` override (not urgent — the bound is time-based and scales naturally across cadences); (e) once deployed, watch whether the still-live-stuck `bill` watermark self-clears (transient-overlap theory) or force-advances at the 2h mark with a critical issue recorded for the 11 Walker Lumber bills — the first real exercise of this code path.

## Session: U-221 — Fan-out idempotency guards (2026-08-10)

**Context:** unit 6 of the QBO audit fix ledger, and the one that stops actual bleeding. U-217's 60s watermark overlap re-pulls records *by design*, but the re-pull is not free at the fan-out tier: every re-projection re-ran the whole fan-out. Split out of U-220 at its Gate 1 precisely because a behavior change on live write paths deserves its own adversarial pass.

**Map corrected the brief on three points, all resolved against prod rather than assumed.** (1) The fan-out is driven by the pull **scripts'** own post-projection loop (`sync_qbo_bill.py:388-461` and its purchase/vendorcredit twins), not by `sync_to_modules` — all eleven scripts do pass `False`, so designing around that flag would have guarded nothing. (2) **The "existing-DriveItem check" the brief named as an obvious lever does not exist.** `ms.DriveItemAttachment` has **0 rows** in prod; `link_driveitem_to_attachment` / `is_attachment_synced` have **zero callers**; `MsOutboxWorker._handle_upload_sharepoint_file` accepts `attachment_id` and never uses it — it uploads and returns, persisting nothing. A registry-based SharePoint guard would never have fired. The completed outbox row is the *only* durable proof in the system that a SharePoint write happened, which is why the SP guard keys on it. (The method's docstring claimed the linkback happened; that false claim — the likely origin of the brief's assumption — is corrected in this diff.) (3) Only **two of four** legs mint junk: Box Excel and MS Excel already no-op at content level (col-Z) *before* writing, so they were deliberately left alone.

**Sized against prod before building.** 10 days: **695 redundant SharePoint uploads** (213 groups / 908 rows — 77% waste) and **358 junk Box versions** (231 groups / 589 pushes — 61%). Since the U-217 deploy: 28 and 31, with same-file push gaps of 7-35 minutes matching the 15-min pull cadence and a 3-push cluster at 22:2x. In that same window only **6** files had a genuinely different sha1 — so a content-hash guard suppresses 31 junk versions while preserving every real update. Also measured, to settle two review claims: max **8** SharePoint-upload outbox rows per entity (avg 1.95 across 1,261 entities), and blob paths are unique per attachment (**1** collision across **28,829** rows).

**Built:** Box guard in `push_blob_to_box` (skip iff exactly one `box.File` row matches folder ∧ name ∧ attachment ∧ `Sha1`; two matches is ambiguous → upload), SharePoint guard in `enqueue_sharepoint_upload` (skip iff a `Status='done'` row's payload matches drive ∧ parent ∧ filename ∧ blob_path ∧ attachment), new `ReadCompletedMsOutboxByEntity` + `read_completed_by_entity`, and `shared/fanout_guard.py` holding the two shared pieces (the kill switch and `same_attachment_id`). Each guard sits at ONE seam, so bill + expense are covered without touching three call sites.

**The reviews found the inverse failure twice — both times through a human repair path, not the pull.** The unit's whole risk profile is that a wrong skip loses a document, and that is exactly where it broke: (a) the guard silently voided `POST /sync/invoice/{id}/box`, the documented repair for a Box file that no longer exists, whose own docstring *relies* on re-upload — fixed with a `force` payload bypass threaded only from that endpoint; (b) `force` was then silently revoked by the Box coalesce's wholesale "latest target wins" payload replacement, and the window is not the 5s debounce but `Status IN ('pending','failed')` — the entire retry lifetime — so any later non-forced enqueue for the same attachment cancelled the repair with a 200 and no error; fixed by making force **sticky** (never revocable by a later caller) plus a `box.outbox.force_refresh_failed` warning when a forced refresh loses the RowVersion race; (c) a third finding, at P1, was that the SharePoint surface had **no bypass at all** — once one `done` row existed, no code path anywhere could re-upload that file. Rather than thread `force` through six call sites with no endpoint to trigger them, that became one operator kill switch (`DISABLE_FANOUT_IDEMPOTENCY_GUARDS`, default off) covering every surface, with `docs/runbooks/fanout-guard-suppressed-upload.md` documenting all three recovery routes.

**Don't trust a guard you wrote — 19/19 mutation matrix.** Both polarity inversions (skip when content *differs*), every identity field dropped individually, the kill switch ignored and inverted, sticky force removed, `same_attachment_id` relaxed to a wildcard, and each guard's exception re-raising instead of falling through to upload. **A harness lesson worth keeping:** the first matrix run left the suite red afterward with clean source — a same-size edit (`== "true"` → `!= "true"`) within one second defeated Python's `(mtime, size)` pyc invalidation, so the stale bytecode survived the restore. That contamination can also manufacture a *false* CAUGHT, so the whole matrix was re-run hermetically with `__pycache__` cleared between every mutation. One mutation then legitimately SURVIVED — but it was my harness mis-targeting `replace(..., 1)` onto the first of two identical strings (the gate inside `enqueue`, which the test patches out); re-aimed at the real branch it was caught, along with its polarity twin.

**Pass 1:** 5 lenses + refute-by-default skeptics (13 agents), 5/5 returned, 0 errored/skipped/empty; 19 raw findings, 4 skeptic-verified, the 15-finding tail recorded rather than dropped. Fix-round re-hunt: 3 lenses scoped to the fix diff, 3/3 returned. **Codex** ran three rounds: CHANGES-REQUESTED → CHANGES-REQUESTED → **PASS** (zero findings, and it verified the two call orderings with runtime probes rather than by reading). Of its round-1 P1s, one was confirmed and fixed (identity comparison outside the guard's `try`, so a raise aborted the push instead of falling through to upload), one refuted on evidence (a both-`None` attachment id still requires a sha1 or blob_path match — that is proof, not an absent field), one confirmed but deferred as its own unit (bill_credit bypasses the outbox), and its P3 refuted on measurement.

**Pass 2 (4 angles) applied:** collapsed a duplicated writes-disallowed branch that carried a second copy of the six-key payload (a drift hazard where adding a field would silently desync local-dev from prod); extracted `same_attachment_id`; single-condition sticky force; docstrings for the new `force` key; test rescaffolding (+13 direct unit tests of the two shared helpers, with an autouse env fixture making the file deterministic). **Rejected, with reasons:** excluding `force` from the coalesce equality domain (a payload differing *only* by force would then compare equal, no update would fire, and the flag would never persist — reintroducing the bug it was meant to tidy); making MS's `attachment_match` lazy (a malformed row would be silently skipped instead of raising, letting a *later* row match and suppress the upload); unifying the two guards (different evidence stores, different return types, different tiers — rule-of-three not met); and adding a pre-download hash (`Attachment.FileHash` is SHA-256, `box.File.Sha1` is SHA-1 — not comparable, and substituting a creation-time hash for "hash of the bytes I am about to send" would weaken the proof to save ~36 blob downloads/day).

**Verify:** **1216 pytest green** (1165 at U-220, **+51**), `py_compile` clean. ⚠️ **SQL step** — `ms.outbox.sql`; all 11 existing procs verified `base==live` (comment-normalized), so re-applying only adds the new read proc. **It is not load-bearing**: if the sproc is missing the guard raises → `except` → enqueue, i.e. exact pre-unit behavior, so a missed migration degrades to the old redundant-upload cost rather than to lost documents. Commit pathspec-scoped; 19 foreign-WIP files from a parallel session left untouched.

**Still owed and still blocked:** `CLAUDE.md` now owes **two** env-var entries (`QBO_SYNC_WATERMARK_OVERLAP_SECONDS` from U-217/U-220, and `DISABLE_FANOUT_IDEMPOTENCY_GUARDS` from this unit), and `docs/runbooks/README.md` owes an index row for the new runbook — both files carry the parallel session's uncommitted WIP, so a pathspec commit would sweep their work into ours.

## Session: U-220 — `sync_from_qbo` returns `SyncOutcome` (2026-08-10)

**Context:** unit 5 of the QBO audit fix ledger, promoted ahead of U-215. Closes U-217 follow-up **(a)** — the item U-217's own notes booked as "a pre-Phase-4 unit". U-217 unified twelve watermark copies but left **three contracts** for "what happened during this pull": seven services took an optional injected `SyncOutcome` accumulator, three took nothing, `reimburse_charge` returned a dict. `coerce_outcome(None)` manufactured a throwaway when a caller forgot to pass one, so the run reported clean and the watermark advanced past records that never landed — the same silent-omission class U-217 spent a unit eliminating at the script tier, relocated one layer down.

**Map found two things the brief didn't name.** (1) `record_synced()` was **already overloaded** — the seven services used it for *staging* upserts while `sync_qbo_item/customer/company_info` used it for *module projections*, so `summary()["synced"]` meant two different things per entity. (2) The in-service `_sync_to_*` helpers swallowed projection exceptions with no recording at all — the same defect one method deeper, reachable only from the routers (all eleven scripts pass `sync_to_modules=False`).

**Shipped:** `SyncOutcome` is now `Generic[T]` carrying `synced: list[T]`; `sync_from_qbo` **returns** it across all eleven pull services; `outcome=` and `coerce_outcome` are deleted; routers read `.synced`. `projected_count` + `record_projected()` de-overload the counter. `reimburse_charge`'s dict and its hand-written adapter are retired. `attachable` and `physical_address` stay out (not watermarked) and are **pinned** as the only exempt `sync_from_qbo` definitions so a twelfth can't drift in.

**The enforcement story is the real lesson of this unit.** The contract change itself was clean — Codex `gpt-5.5 xhigh`: no P0/P1/P2; a 5-lens adversarial hunt: 9 raw findings, **0 survived refutation**. Everything of consequence was found by **mutation-testing the guards instead of trusting them**:
- The first guard asserted the committed outcome was "ever bound from the service" — an **ANY** check. Rebinding (`outcome = SyncOutcome()` after the call) passed the guard **and all 1158 tests** while committing an empty outcome. Hardened to an **ALL** check over every binding in the enclosing function.
- The re-hunt then defeated *that* three more ways: **name-laundering** through `sync_qbo_to_local` (trusted by name, never by return value), **`push_run` aliasing** (`push_run = run`), and **glob escape** (move the commit out of `scripts/sync_qbo_*.py` — precisely what a "lift the duplicated commit into a helper" refactor does). Each verified end-to-end with the suite green.
- Patching an AST denylist a third time was the wrong altitude, so enforcement moved to a **runtime provenance stamp**: `SyncOutcome.for_service_pull()` sets `from_service_pull`, and `WatermarkRun.commit` refuses an unstamped outcome **before** the skip branch (so `skip=True` can't become a laundering hole). Push watermarks got a real verb — `commit_push()` — which deleted the dummy `SyncOutcome()`, the `allow_unstamped` escape hatch, and the test policing it.
- Pass 2 then collapsed the whole 219-line binding analyzer into **one flat rule**: no sync script may construct a `SyncOutcome` at all. Every defeat requires constructing one, so the simpler rule is **strictly stronger** — it catches the helper-laundering case the 219-line version missed. It also killed a **fail-open** degradation: the old non-vacuity check recovered line numbers by re-parsing its own error prose with a bare `except ValueError: continue`, so rewording a message silently shrank the offender set and the check kept passing.

**Mutation matrix, final: 7/7 caught** — literal `SyncOutcome()` at commit, rebinding, helper laundering, push-hatch disguise, glob escape, ladder inversion (the U-217 landmine), and dropping the stamp from one service. Proven separately at runtime: a laundered outcome now **raises with zero writes** instead of silently advancing.

**Pass 2 (4 angles) also:** deleted 10 redundant `logger.error` lines that double-logged every projection failure **with the wrong id keyspace** (staging PK `vendor.id` vs `vendor.qbo_id`) and printed ERROR-then-INFO for one event; reworded the shared log to state *classification* not *consequence* (it promised "watermark holds for retry" on the router path, which commits no watermark); re-privatized `_record_skip` and gave the staging case its own `record_staging_skip`; dropped duplicate local counters in the five single-projection scripts (**not** customer/item/term, whose split counts are load-bearing — deleting those would have silently changed admin envelope numbers).

**1165 pytest green (+14 over U-217's 1151).** 34 files, `+766/−406`. **No SQL step** — pure Python contract change.

**Booked as follow-ups, not built:** identity-based provenance (`run.pull(...)` asserting `outcome is self._pull_outcome`, which makes the boolean and the remaining guard unnecessary — a flag that needs a source-code police force is a permission wearing data's clothes); a shared `base/projection.py::project_all` collapsing ten near-identical `_sync_to_*` bodies; `sync_qbo_to_local` returning `SyncOutcome` alone plus a `pull_result_dict()` builder (deferred — those dicts are admin response bodies); `SyncFailure(qbo_id, reason)` so `record_*_failure(id, error)` stops accepting a diagnostic and dropping it (**`reimburse_charge` regressed here**: N malformed records now collapse to N identical `"<no-id>"` strings, losing the raw payload the old `skipped_no_id` carried); a shared `tests/_ast_utils.py`.

**Still owed and still blocked:** `CLAUDE.md` needs `QBO_SYNC_WATERMARK_OVERLAP_SECONDS` in its App Service env-var list — a parallel session holds uncommitted `CLAUDE.md` WIP, so a pathspec commit of that file would sweep their work into ours. Same blocker as U-217.

## Session: U-217 — Watermark integrity, fleet-wide (2026-08-10)

**Context:** unit 4 of the QBO audit fix ledger. Covers addendum **S-01** (staging-tier failures invisible to the hold), **S-06** (six reference pulls advance past swallowed projection failures), **P1-07** (invoice pull has no hold at all) and the **once-verified overlap P1** (watermark is end-of-run wall clock). Load-bearing for U-214: its new raise paths were honest but, with an unconditional advance, every raise was a permanent skip.

**Map corrected the brief twice.** (1) There were **twelve** hand-copies, not eleven — `sync_qbo_company_info.py` inlined the whole get-or-create *and* the `Sync(...)` rebuild in its main body (plus a stray `print`), so it never showed up in a grep for the helper names. (2) The audit lumped all ten services together, but only **seven** actually swallow staging failures; `customer`, `item` and `company_info` have **unguarded** upserts that raise straight out of `sync_from_qbo`, which is already fail-loud and already prevents the advance. Wiring those three would have been noise. Also found undocumented: `ReadSyncs` is the one sproc that returns `[LastSyncDatetime]` **uncast** (every other column is `CONVERT(VARCHAR(19),…)`), so the field is a `datetime` on read and a `str` on write — `_normalize_last_sync` is typed `Optional[str]` and would have `AttributeError`d on it.

**Prod evidence gathered before building:** zero vendor-orphan `ReconciliationIssue` rows since the U-214 deploy (those paths hadn't fired yet), but **792 Purchases, 258 Bills and 7 VendorCredits** sitting in `qbo_missing_locally` — the realized form of the same defect. `dbo.Sync`: 11 rows, **0 duplicates**, so the unique index applies cleanly.

**Built:** `SyncOutcome` (`base/sync_outcome.py`) as the one failure vocabulary both tiers record into, and `WatermarkRun` (`scripts/sync_helper.py`) as the one runner — `open()` captures `query_start` **before any I/O**, resolves the row duplicate-safely through new **public** repo methods (`read_candidates_for` / `pick_canonical` / `watermark_is_at_or_ahead`), and `commit()` applies a single ladder: **skip > hold > end_date > query_start − overlap**. The hold outranking `end_date` is the fix for bill and vendorcredit, which tested `end_date` *above* the failure check, so a failed historical batch advanced anyway. All twelve copies deleted; the eleven scripts went **445 in / 1060 out (−615 net)**. `UNIQUE(Provider,Env,Entity)` added to `dbo.sync.sql`, guarded on *both* `sys.indexes` and the absence of duplicates so re-applying the base file can never roll back the seven `CREATE OR ALTER PROCEDURE` batches ahead of it in the same transaction.

**Deliberately NOT done:** `sync_from_qbo` keeps returning `List[...]` with an additive accumulator rather than converging on `reimburse_charge`'s dict. Converging means editing nine routers that all do `[x.to_dict() for x in result]` — a wide mechanical edit by a fast model, which is precisely the "landed in two of three copies" failure this unit exists to stop. Booked as a pre-Phase-4 unit instead, where it collapses three contracts into one.

**Pass 1 (Workflow, 4 lenses + refute-by-default skeptics; 16 agents).** 4/4 lenses returned, 0 errored/skipped/empty — no dead lens this time. 22 deduped findings; the verify cap covered 6, and the uncovered tail is recorded below rather than silently dropped. **Codex** (independent, `gpt-5.5` xhigh) ran twice and found the two most valuable things in the unit.

**Four defects this unit *introduced*, all caught and fixed:**
- **`--push` TypeError (P1).** `WatermarkRun.last_sync_time` returns a string; `sync_local_to_qbo` compared it against a `datetime`. Reproduced. It would have failed every `--push` run *and*, crashing upstream of `commit()`, left the pull watermark unadvanced so each run re-pulled and re-enqueued the same Excel/SharePoint/Box work forever. Fixed by normalizing once at the top of the function — which also closes a pre-existing crash on the first-ever push run (the `start_time_str` fallback was already a string).
- **A brand-new silent-skip path (P1).** Codex flagged `ValueError → permanent skip` in `purchase`; tracing it, its framing was wrong (that split was pre-existing there) but the truth was worse — the conversion **added** that branch to *six* scripts that previously had a blanket `except Exception`. Connectors do `raise ValueError(...) from e`, so transient DB errors became permanent skips.
- **An inverted safe default (P1), introduced by the fix for the above.** Routing everything through a transient-*proving* classifier meant `QboTimeoutError`, `QboRateLimitError`, `QboTransportError`, `QboServerError` and bare `RuntimeError` (the line-aggregation raise) all classified as **skip** where they previously **held**. Fixed by moving the policy to `base/errors.py::is_retryable_error`, unioning **both** transience vocabularies (`QboError.is_retryable` ∪ `shared.database.is_transient_error`) over the `__cause__`/`__context__` chain, and inverting the default: **unknown errors hold.** A wrong hold costs one idempotent re-pull; a wrong skip loses the record until a human edits it in QBO.
- **Query timeouts still skipped (P1, Codex round 2).** `shared.database`'s vocabulary has `40001` and connection errors but not `HYT00` / "Query timeout expired" — the single most likely DB failure under load. Closed with a QBO-local supplement in `errors.py`, deliberately **not** in `shared/database.py`, since widening that list changes `with_retry` for every caller in the app.

Final classification matrix — only a genuinely permanent `ValueError` skips; QBO retryables, bare `RuntimeError`, and `ValueError` wrapping `40001`/`HYT00`/`HYT01`/connection-reset all **hold**.

**Pass 2 (/simplify, 4 angles).** Applied: deleted the dead code the unit itself shipped (`get_or_create_sync_record`, the unused `read_by_provider_env_entity` pair, three orphaned `_parse_datetime` copies ≈120 lines, eight unused imports); made the repo the single owner of the tie-break rule so `sync_helper` stopped importing repo **privates** across a package boundary; separated the **id keyspaces** in the envelope (`failed_*_ids` = local `qbo.*` PKs, `staging_failed_qbo_ids` = QBO API ids — CLAUDE.md flags mixing these by name); deleted `buckets_total()`, a documented invariant already false for `company_info`; and collapsed the unit's own irony — thirteen copy-pasted skip/failure logging branches (a copy-pasted classification branch replacing a copy-pasted watermark helper) into `record_projection_error(label=, logger=)`, with the shadow failure lists deleted so bookkeeping lives only in `SyncOutcome`. **Declined:** collapsing `end_time_str`'s normalizer (would change a visible envelope value — Pass 2 must not change behavior).

**One deliberate constant change, on evidence:** overlap default **120s → 60s**. The efficiency review measured the real cadence (four transactional pulls every 15 min = 96 ticks/day each) and disproved my Gate-1 assumption that re-pulling is free: at the fan-out tier a re-projected bill re-enqueues an **unguarded** SharePoint upload (full blob download + full PUT), a Box doc-push that sends the same bytes twice and mints a junk file version, and a Box workbook lock + full-`.xlsx` download that then no-ops. 60s halves the re-scan while keeping ~60× margin over clock skew, which is the only thing the overlap must cover (`query_start` already covers the run's own duration).

**Verify:** **1151 pytest green** (1099 baseline at U-214, **+52 new**), `py_compile` clean, all 11 scripts import. Verified offline that the runner's watermark string round-trips to a byte-identical QBO filter vs the pre-U-217 path, and against SQL Server that the `Z`-suffixed value converts to `DATETIME2(3)` with no shift. Codex final verdict **PASS**, zero findings. ⚠️ **SQL-FIRST deploy** — `dbo.sync.sql` must be applied before the container; all 7 Sync sprocs verified `base==live`, so re-applying only adds the index. Commit pathspec-scoped; 19 foreign-WIP files from a parallel session left untouched.

## Session: U-214 — Vendor connector hardening (2026-08-10)

**Context:** unit 2 of the QBO audit fix ledger (after U-211 metering/breaker and U-212 mass-delete cluster). Covers audit **P1-08** (delete-mapping-on-empty-read → duplicate vendor or detached sync) and **P1-09** (unconditional `Vendor.Name` clobber from QBO DisplayName). The highest-leverage reference mapping in the system — every Bill/Purchase/VendorCredit pull resolves vendors through `qbo.VendorVendor`.

**Map phase resolved three things rather than assuming them.** (1) **The mapping HAS a repoint sproc** — `UpdateVendorVendorById` (RowVersion-checked, CASE-WHEN guarded) + `VendorVendorRepository.update_by_id` — so this unit got the full **Project/U-022 in-place-repoint** template, not the degraded raise-only shape U-029/U-031 had to use for lack of one. (2) **The natural fingerprint is exact `Vendor.Name == DisplayName`** via `ReadVendorByName` (`TOP 1`, `IsDeleted = 0`) — the very predicate `VendorService.create` already dedups on, and backed by the filtered unique index `UQ_Vendor_Name_Active`, so it is at-most-one by construction (stronger than Project's `(Name, CustomerId)`). (3) **Nothing depended on the clobber** — two callers only, both per-record `try/except`, and `sync_local_to_qbo` is a stub.

**Built:** (a) heal-don't-delete — re-resolve by name, **repoint the mapping in place** when it rebinds, else record a critical `qbo.ReconciliationIssue` (`orphaned_vendor_vendor_mapping` / `duplicate_qbo_vendor`, failure-isolated insert) and RAISE, mutating nothing; the `delete_by_id` + fall-through-to-create is gone. (b) `preserve_human_edited_name` — new pure sibling in `base/field_ownership.py` (deliberately NOT `preserve_human_edited_ref`: there is no `QBO-<id>` placeholder for a name, so a `qbo_id` param would encode a dead branch) + a `VENDOR` entry in the ownership registry with `name` as `both_editable`. (c) mapping-create failures raise instead of being swallowed. (d) **adopt-an-existing-unmapped-Vendor-by-name before create** — not in the literal finding text, but (c) is incoherent without it: `VendorService.create` refuses a duplicate name, so a raised mapping-create would strand the vendor permanently. (e) the name write is **skipped when unchanged** — name is this connector's only mapped field, so preservation turned the UPDATE into a pure no-op; skipping it drops one write transaction + `ROWVERSION`/`ModifiedDatetime` churn per vendor per pull (~1,100 no-op write transactions eliminated on a full re-sync, and one less 409 source for the vendor agent and compliance sweep, which both write `dbo.Vendor`).

**Pass 1 (Workflow, 4 lenses + refute-by-default skeptics; 33 agents).** 12 raw findings, **2 confirmed** (both mutation-verified test-hardening P3s: `delete_by_id.assert_not_called()` missing from three branches; the reconciliation recorder's failure-isolation `try/except` never exercised — the two sibling heal suites both ship that test), **10 refuted**. The most useful refutation: *"heal-by-name is inert for curated vendors"* died on the observation that the **pre-U-214 code clobbered names for the entire mapped population**, so local names equal DisplayName today — divergence only begins after this ships. **The control-flow lens stalled on all 6 attempts and returned nothing**; re-run standalone with a bounded 15-tool-call state matrix, which cleared 8 of 9 states and found **2 real P3s** the first pass missed:
- **blank-name write:** when the stored name is blank AND QBO supplies no DisplayName, `resolved_name` is `None`, differs from `"   "`, and the code took the WRITE branch — `dbo.UpdateVendorById` sets `[Name] = @Name` **unconditionally** (every sibling column has a `CASE WHEN`; `[Name]` does not) against `NVARCHAR(450) NOT NULL` → SQL 515, or silent blanking with `display_name=""`. Guard tightened to `if resolved_name and ...`.
- **whitespace-padding mismatch:** the connector looked up the RAW DisplayName while `VendorService.create` strips and dedups on the STRIPPED value, so `" Acme Supply"` missed the adopt branch and then collided inside `create()` — reproducing exactly the permanent detachment adopt-by-name exists to prevent (and the mirror case minted a near-duplicate, since `UQ_Vendor_Name_Active` sees different strings). Fixed by normalizing once at the top.

**Codex** (independent, `gpt-5.5` xhigh): round 1 **CHANGES-REQUESTED** on one P1 — vendor projection failures are still watermarked as successes. Traced and **confirmed in the real code** (`_update_sync_record` runs unconditionally), but it is the reference-pull watermark hold already booked as **U-217** and declared out of scope. What WAS in scope: two code comments and an operator-facing log string promised a "next tick" self-heal that cannot happen — corrected to state the permanent-skip truth and point at the `ReconciliationIssue` row as the durable follow-up. Round 2 verdict: **PASS**, zero findings at any severity.

**Pass 2 (/simplify, 4 angles).** Applied: guard simplified to `if stored_value and stored_value.strip()`; the helper's docstring re-led with the GENERAL contract (it lives in shared `base/` — a Vendor-only docstring is how the next reference pull ends up hand-copying it) and now records that it and `preserve_human_edited_ref` **disagree on what "blank" means** (falsy vs `.strip()`); `_coerce_id` applied to the four hand-inlined copies remaining in the same file; the no-op-path debug line stopped claiming a curated name was "preserved" when both names were blank; test-mock builder collapsed from a 6-tuple with two permanently-dead slots to the connector alone; two pairs of near-duplicate tests parametrized (which also closed a real gap — whitespace-only stored name + a real DisplayName was untested); the field-ownership registry test shrunk from a dict-literal echo to the part that can actually fail. **Declined with reasons:** collapsing the heal and adopt branches (they diverge in the tolerance check, the tail, and the message facts — would need a 3-tuple return against a documented unpack hazard to save six lines; the sibling connector keeps them separate for the same reason); moving name preservation into the sproc (**a `CASE WHEN @Name IS NULL` guard would not have fixed P1-09 at all** — the connector passed a deliberate non-NULL *wrong* value, so the guard's ELSE branch fires every tick; it would also cost a SQL migration + deploy ordering and buy nothing, since the only other caller never passes NULL); extracting the fourth heal copy into a shared helper (five files across another session's WIP tree, and it would bury a P1 fix inside a refactor — booked, with the four copies now re-scoped into two families).

**Verify:** **1099 pytest green** (1083 baseline at U-212, **+16 new**), `py_compile` clean on all three files. **No SQL step for this unit** — verified against prod that all four sprocs the new paths call are PRESENT and `UpdateVendorVendorById` is **base==live**, so it is container-only. Commit pathspec-scoped: 19 foreign-WIP files from a parallel session's email-teardown work left untouched.

**Owed:** deploy — now **three units deep** (U-211, U-212, U-214) and **order-dependent**: U-211's `python scripts/run_sql.py integrations/intuit/qbo/base/sql/qbo.api_usage.sql` must land BEFORE the container build. Follow-ups booked to `TODO.md`, the largest being `_ensure_vendor_address`'s pre-existing full-table `read_all()` per vendor (~1,100 full-table reads on a full re-sync) when `read_all_by_vendor_id` already exists.

## Session: U-212 — QBO mass-delete cluster fix (+ absorbs U-213 fault-610) (2026-08-09)

**Context:** first unit of the QBO audit fix ledger (TODO.md; board U-212–U-218 booked this session from the 2026-08-07 evaluation's findings). Covers audit P1-02/05/06/12 + the delete-order-zombie P2 + P1-11 (U-213, absorbed).

**Built:** (1) `base/delete_reconcile.py` — the strict pull-side delete gate: live ids from the strict pager, `QBO_PULL_DELETE_MAX_CANDIDATES` ceiling (50), per-candidate GET confirm (only `QboNotFoundError` confirms), systemic-error abort (budget/rate-limit/auth → delete nothing), both diff sides through `normalize_qbo_id`, ceiling-abort + partial-delete written durably to `[qbo].[ReconciliationIssue]` under the monitored `qbo.reconcile.issue.flagged` event. (2) Bill/Purchase/VendorCredit `_reconcile_deleted_*` rewired onto it (no longer trust the lenient list; `mapping_removed` arms on line-mapping deletes too). (3) **Client: fault 610 → `QboNotFoundError`** — Intuit signals hard-deleted txns as HTTP 400/610 not 404; one shared-client branch fixes the void detectors AND powers the pull confirm (this IS U-213). (4) **Client: empty/unparseable 2xx now raises retryable `QboMalformedResponseError`** instead of silent `{}` — kills the silent-truncation root enabler. (5) Report router maps `QboNotFoundError` → 404.

**Pass 1:** 3-lens workflow + a standalone blast-radius agent (the workflow's blast lens stalled; relaunched direct). Verdict on the malformed-2xx change: **SAFE** — every lenient pager's `{}`-as-end-of-pages is exactly what's being removed; scripts' top-level catches hold watermarks; on push paths the old behavior was WORSE (empty-body 2xx → pydantic error → instant dead-letter with the QBO write applied; now retried under the same requestid → Intuit dedup). company_info flips from wrongly-silent success (watermark advanced past a masked update) to a correct loud failure. 2 P3s fixed: systemic-error abort in the confirm loop; report-router 610 → clean 404. 2 P3s refuted (whitespace-id false-diff had no producer; attachable 610 gap has no catcher and dies in the booked re-route).

**Pass 2 (/simplify, 4 agents):** applied — normalized diff per the `ids.py` contract (reuse + the refuted-P3's hardening half), empty-candidates early exit before the metered live fetch, dropped the test-only `max_candidates` param (env is the one knob), `record_partial_delete_issue` extraction (killed the 3× template), monitored event name on issue writes, flattened the client try/except, `entity_type` rename, stale docstrings fixed, dead `TypeError` branch + unused `Decimal` removed. **Backported the systemic-abort to the three void detectors** (reuse+altitude both flagged the live divergence: the gate had the fix, the detectors would still burn ~200 metered GETs against a tripped breaker). **`SELECT Id` projection in the strict pager** (adjacent pre-existing; U-212 tripled its call sites; ~50-100x payload cut; no test pinned the query). Skipped with notes: gate-invocation preamble extraction (deliberate per-entity cascades), double-paging on full sync (accepted cost), shared env-parser (booked), account-service strict fetch (booked → U-218).

**Verify:** 1083 pytest green (18 new in `tests/test_qbo_pull_delete_reconcile.py`), py_compile clean. **Deploy:** container-only (no SQL in this unit) — but U-211's `qbo.api_usage.sql` is still owed first from the prior session.

## Session: U-211 — QBO API metering + monthly-budget breaker (staging-removal Phase 1) (2026-08-07)

**Context:** same session ran the full QBO integration evaluation (38-agent workflow → `docs/audit_qbo_integration_2026_08_07.md`, 14 double-confirmed P1s, fixes HELD) and the staging-removal architecture decision with Chris: **one-copy architecture** — QBO live truth, `dbo.*` the single working copy, `qbo.*` typed staging data tables to be deleted over 6 phases (program recorded on `build.one.team/BOARD.md`; scope explicitly confirmed `qbo.*`-only after Chris's message said "dbo"). U-211 is Phase 1: without a durable meter + breaker, the live-read architecture would be one runaway loop away from repeating July 2026's five-day Intuit hard-block.

**Built:** `[qbo].[ApiUsage]` (one row per realm+month; atomic UPDATE-first/INSERT-on-miss/2601-catch upsert, `SET LOCK_TIMEOUT 2000`, OUTPUT-INTO + single final SELECT) + `base/budget.py` (`QboApiBudget.record_call/record_call_or_raise/status`, `BudgetStatus` with derived `blocked`/`warning` properties, fail-OPEN on meter DB errors, band-crossing log dedup, env thresholds: `QBO_MONTHLY_CALL_BUDGET` 500K / block 0.95 / warn 0.80 / `QBO_BUDGET_ENFORCE` kill switch). Wired at the two HTTP choke points: `QboHttpClient._send_http` (every round-trip incl. 401-resend + retry attempts — mirrors Intuit's counting) and `QboAttachableClient` (4 send sites; pre-signed download + OAuth/discovery deliberately unmetered). Worker: `drain_once` up-front skip while blocked (rows stay pending, zero attempts burned); mid-drain trip parks the row via `mark_failed(next_retry_at = reset_at_for_month(error.month_key))` — never dead-letters on budget. `QboBudgetExceededError` is `is_retryable=False` (no in-process spin) with the worker isinstance-park BEFORE the is_retryable branch.

**Pass 1 (adversarial workflow, 3 lenses + refute-by-default):** 6 confirmed findings, 4 fixed in-unit — hot-row lock wait had no timeout (→ `SET LOCK_TIMEOUT 2000`, converting a stalled-holder freeze into fail-open); budget trip during bill attachment sync was swallowed by TWO best-effort except layers → row marked done with attachments silently dropped (→ `QboBudgetExceededError` re-raise carve-outs in `BillService.push_to_qbo` + `_sync_attachments_to_qbo`); park date used handler wall clock not the exhausted month (→ `reset_at_for_month`); attachable metering had zero tests (→ 4 added). Booked: no mid-month unpark path (manual `UPDATE … WHERE LastError LIKE 'Parked:%'`), per-realm vs cross-realm threshold inconsistency (latent, single-realm today), dead `FileAccessUri` download fallback. Self-caught pre-review: first-insert-of-month returned count 0 through the empty first result set (fixed at sproc level in Pass 2's shape change).

**Pass 2 (/simplify, 4 agents):** applied — `BudgetStatus.blocked/warning` as derived properties (killed the `_permissive_status` near-clone), ONE shared `record_call_or_raise` home for the refusal (both clients were carrying verbatim 13-line raise blocks), constructor-injected `api_budget` on the attachable client (normalized the DI seam), sproc reshaped to single-result-set (retired the pyodbc `nextset` walk + its test), dropped speculative `delta`/unused `realm_id` params, env-parse fast path, `message==event_name` log convention, import hoist in bill service. Skipped with notes: router/script attachment-loop clone collapse (altitude found the "second site" for the carve-out ALREADY exists at `bill/api/router.py:210` + `sync_qbo_bill.py:776` — booked, outside diff), `drain_all` check hoist (no prod caller), blocked-month status caching (would break manual mid-month DB reset). Efficiency flagged for its own unit: verify ODBC pooling is actually live in prod + `lru_cache` on `_get_db_config` (the meter doubles `get_connection` frequency on QBO paths; pooled ≈1-3ms, unpooled ≈20-60ms per call).

**Verify:** 1066 pytest green (26 new in `tests/test_qbo_api_budget.py`), py_compile clean. **Docs:** runbook `docs/runbooks/qbo-api-budget-breaker.md` (runbooks README index entry deferred — file is foreign-WIP). **Deploy (SQL FIRST):** `python scripts/run_sql.py integrations/intuit/qbo/base/sql/qbo.api_usage.sql` must land before the container (the client calls `IncrementQboApiUsage` on every QBO request; missing sproc = fail-open with loud logs, not an outage — but apply first anyway). Then ACR build `:latest` + restart. Optional env overrides need nothing set (all defaults live).

## Session: U-164 — SET NOCOUNT ON on three contract-labor sprocs (guard pins five) (2026-07-28)

**Premise refuted:** `ReadContractLaborDailySummary` works correctly in prod today — assignment-only `SELECT @var = …` does NOT emit a blocking count-only result on the prod stack (pyodbc 5.2.0 / ODBC Driver 17 / Azure SQL). Verified with real data on the live `/daily-summary` endpoint.

**Real pyodbc break shape:** DML (INSERT/UPDATE/DELETE) followed by a row-returning SELECT, with the caller doing `fetchone()`. Verified raising against prod for `UpdateContractLaborStatusByIds` with `@Ids='-1'` (zero-row, no write). `SET NOCOUNT ON` suppresses the rows-affected DONE token but does NOT affect `@@ROWCOUNT` (verified: 0-row UPDATE → 0, 3-row INSERT → 3).

**Changes:** `entities/contract_labor/sql/dbo.contract_labor.sql` — `SET NOCOUNT ON` added to `UpdateContractLaborStatusByIds`, `DeleteContractLaborLineItemsByContractLaborId` (both currently dead code — no callers — fix is preventive), and `ReadContractLaborDailySummary` (hygiene + removed pointless read-only `BEGIN TRANSACTION`/`COMMIT`). Guard test `test_sproc_body_pins_set_nocount_on` (`NOCOUNT_PINNED_SPROCS`) pins five: the three changed here plus `FindContractLaborForReviewerReply` and `UpdateContractLaborAggregates`, which already carried `SET NOCOUNT ON` and exhibit the same DML → row-returning-SELECT → `fetchone()` shape on live routes. The read-only `BEGIN TRANSACTION` removal is pinned separately via a `SPROC_BODY_FORBIDDEN_PATTERNS` row. Docs corrected in README, stub comment, and prior SESSION_NOTES claims.

**Deploy:** SQL-apply only — a surgical extract of just the three changed `CREATE OR ALTER` blocks into an uncommitted scratchpad `.sql` run through `scripts/run_sql.py` (NOT the whole base file; a committed migration copy is impossible because the whole-file single-source guard would fail). No container deploy. Base==live maintained after apply.
...
**Client workbook profile (de-risks openpyxl):** all .xlsx; DETAILS tab = plain data (the only tab we edit); summary tabs = formula-based; NO pivots/charts/macros anywhere. So openpyxl is fidelity-safe IF load data_only=False (preserve formulas) + set fullCalcOnLoad (recalc on open). NOTE: openpyxl rewrites the WHOLE file, so this only holds because the whole workbook is chart/pivot/macro-free — not just the DETAILS tab.

**Built (integrations/box/excel/):** workbook_editor.apply_rows_to_details (openpyxl; data_only=False; fullCalcOnLoad; col-Z (index 25) public_id idempotency; SubCostCode insertion ported from bill sync_to_excel_workbook; 26-col row A..Z; Decimal-safe col N; returns bytes=None no-op when all keys present), row_builder.build_details_rows (per-entity bill/expense/bill_credit), BoxExcelUpdateService drain handler (box_app_lock → WOPI-lock defer (BoxLockedError retryable) → Box lock PUT if_match=etag0 → capture etag1 → download → apply → upload_file_version if_match=etag1 → [box].File(Kind=workbook)+PushLog → best-effort unlock in finally), BoxProjectWorkbookService + repo + router (POST /box/map-workbook, GET /box/project-workbooks, QBO_SYNC-gated), box.project_workbook.sql ([box].[ProjectWorkbook], deployed to prod DB). enqueue_box_excel + KIND_UPDATE_EXCEL handler in worker. Completion-flow enqueues in bill/expense/bill_credit (additive, failure-isolated, ALLOW_BOX_WRITES-gated, lazy imports — Phase-2 discipline). Runbook box-excel-conflict-storm.md.

**Build note:** ran via workflow; the connection drop killed 2 agents (excel-core builder + box-api-contract reviewer) AFTER excel-core wrote its files. Filled both gaps manually: verified the etag/lock dance, payload-key↔handler match, repo sproc-name↔SQL match, gate, SQL discipline (the 2 "ROLLBACK" greps were comments only — zero in-proc rollback). openpyxl-fidelity + surgery-safety reviewers both APPROVED (fidelity reviewer independently confirmed at raw-XML that the formula <f> node + fullCalcOnLoad=1 survive and the stale cached value is dropped).

**LIVE-VERIFIED on the Rogers Build test bed (CCG service account, ALLOW_BOX_WRITES=true in-process only):** real Box round-trip (upload .xlsx with DETAILS + `=SUM(DETAILS!N:N)` summary → lock → download → openpyxl edit → upload-version → re-download) preserves the summary formula as a formula + fullCalcOnLoad=True; new col-Z rows present; idempotent re-apply = no-op; WOPI-lock detection correct; lock/etag/if_match works (note: taking a lock did not bump the etag in practice — handler captures whatever the lock response returns, so it's correct either way). Repo CRUD against deployed [box].[ProjectWorkbook] OK. openpyxl 3.1.2 is in requirements; local runs use .venv/bin/python (system python3.9 lacks it).

**STATUS:** committed+pushed (79580b0); ships DORMANT (ALLOW_BOX_WRITES unset in prod, no workbook mappings). [box].[ProjectWorkbook] schema IS in the prod DB. **NOT yet deployed to the prod IMAGE** (prod runs f89f2b7; Phase 3 = 79580b0) — deploy is a separate explicit step. Invoice Excel deferred (DETAILS = vendor cost lines). **OPS: I added a temp Azure SQL firewall rule `claude-session-temp-1781539334` for IP 174.252.147.194 (my IP changed after the reconnect) on server bchristopher/owner — remove it when convenient.**
...
**Auth = CCG (Client Credentials Grant), radically simpler than QBO/MS:** no refresh tokens, no DB Auth table, no sp_getapplock on mint. `BoxAuthService` (auth/business/service.py) mints 60-min tokens on demand from `box_client_id/secret/enterprise_id` (config.py, App Service settings) with a process-wide double-checked in-memory cache (threading.Lock = stampede guard only — CCG mints are concurrency-safe). Token-endpoint 429 → retryable `BoxRateLimitError` w/ Retry-After; 400/401 → non-retryable `BoxAuthError` (config problem).

**base/ package** mirrors ms/base exactly (errors/retry/locking/logger/correlation/idempotency/client). Box-specific deltas: `BoxConflictError` carries `context_info` (409 → conflicting file id for the version-upload recovery path); `BoxPreconditionError` (412, etag stale) is non-retryable at the HTTP layer by design — Phase 2 outbox handler refetches+reapplies; 403 splits on the structured error code (`item_locked` etc. → retryable `BoxLockedError`; message-text sniff only when no code present); 202+Retry-After (download not-ready AND generic JSON path) → retryable `BoxServiceUnavailableError`; retry layer clamps a server-advised Retry-After into the remaining budget for one final attempt instead of hard-failing. Write gate `ALLOW_BOX_WRITES` (os.getenv, default-deny) enforced pre-network on POST/PUT/DELETE + both multipart upload methods (attributes part FIRST — Box hard-400s otherwise); token mint deliberately NOT gated. No Graph-style request-id dedup header exists on Box — idempotency module is forward scaffolding for the Phase 2 outbox.

**Routes** (mirroring /ms/auth, gated `Modules.QBO_SYNC` per MS precedent — no new Module): `GET /api/v1/box/auth/status` (config booleans + token cache state + write-gate state; never secrets), `GET /api/v1/box/auth/test` (mint + `GET /users/me`). Wired in app.py (import line 102, include line 219).

**Verified:** py_compile on all 14 files + app.py/config.py; package import + sanity assertions; behavior tests (Retry-After clamp, budget abort, single-mint cache + force_refresh, mint-429 mapping) all pass under system python3.9 (full `import app` needs 3.10+ syntax from other modules — pre-existing local-env limitation; prod Docker is 3.11).

**Live smoke 2026-06-12 (next day, same session):** credentials landed in `.env` (first paste was 2FA backup codes — 3 comma-separated 15-char uppercase codes, diagnosed by shape without printing values; real secret is alnum32). **CCG mint + users/me WORKS through the spine**: app authorized in enterprise "Rogers Build" (1492747040); service account `AutomationUser_2594665_YpKO8TK6sF@boxdevedition.com` ("build.one"), sees 0 root items (not collaborated yet — expected). **Write-mechanics validation 8/8 PASS** on a fresh dev token (60-min lifetime — first one expired) against Chris's own account via BoxHttpClient with gate temporarily opened in-process: folder create, multipart upload (sha1 verified), duplicate-name 409 → BoxConflictError.context_info recovery (id matched), version-upload with current etag, stale etag → BoxPreconditionError(412), lock set/clear with expires_at, redirect download (bytes verified), version list; throwaway folder deleted.

**Tenant model resolved (2026-06-12):** Rogers Build is itself a client of build.one; its Box enterprise (1492747040) = permanent DEV/TEST BED. Production destination = the actual client's separate enterprise — zero code change to retarget (authorize same Client ID in their Admin Console + swap BOX_ENTERPRISE_ID; per-enterprise service accounts isolated).

**Phase 2 BUILT + LIVE-SMOKED (2026-06-12, same session, 5-builder + 3-reviewer workflow):**
- **SQL deployed to prod DB** (4 files via run_sql.py): `[box].[Outbox]` (+CreatedByUserId DEFAULT 17; claim sproc increments Attempts AT CLAIM — worker must NOT +1 again), `[box].[Folder]` + `[box].[ProjectFolder]` (1:1 adopt-only; result sets alias the BIGINT FK as `[FolderId]`, name `[BoxFolderId]` reserved for Box's string id), `[box].[File]` + `[box].[PushLog]` (audit registry), `[box].[ReconciliationIssue]` (ms column set; TenantId carries enterprise id, DriveItemId carries Box ids). All sprocs in dbo, SET NOCOUNT ON, no in-proc ROLLBACK.
- **Python**: `integrations/box/{outbox,file,folder,reconciliation}/` — enqueue gate (ALLOW_BOX_WRITES + ALLOWED_PUSH_CLASSES allowlist + sanitize_filename w/ entity-hash + per-attachment Policy C coalesce + CreatedByUserId from ContextVar); worker (authz save/set/restore, auth circuit-breaker pre-pass, 3-consecutive visibility circuit, MS backoff, dead-letter→critical issue, **PAUSE_BOX_DRAIN honored inside the worker** so scheduler tick + endpoint share one guard); upload handler (blob→sha1→upload→**409 registry-verified version recovery**→sha1 verify→File+PushLog); folder mapping service + `POST /api/v1/box/map-project` + `GET /api/v1/box/project-folders` (QBO_SYNC-gated).
- **Completion-flow surgery** (additive, failure-isolated, lazy imports, early-return when gate closed): bill (complete_bill Step 6), expense (Step 5 — single attachment pass; receipt pass dropped as coalesce no-op), bill_credit (complete_service), invoice packet (router helper — covers manual generation + complete_invoice).
- **Ops**: `POST /api/v1/admin/box/drain` (own endpoint — never shares the QBO/MS tick; pause shape matches PAUSE_EMAIL_*), APScheduler fallback job, `scripts/retry_box_outbox_dead_letters.py`, runbooks box-{auth-reauthorization,rate-limit,outbox-backlog-growing}.md + README index. Scheduler repo: `drain_box_outbox` timer (30s).
- **Review fixes applied**: reserved-`filename` LogRecord key (would have crashed every successful push), Attempts double-count, PAUSE on the scheduler path, folder-repo FolderId mapping, schema guards in all 4 SQL files, expense receipt no-op, gate early-returns in all 4 helpers.
- **LIVE E2E PASSED on the test bed**: outbox row 1 enqueued→drained→uploaded (sha1-verified, `phase2-smoke-bill-a6dcf1ce.pdf`)→File+PushLog rows→done@attempt 1. Idempotent re-run: 409→registry check→version upload→PushLog 2, Box shows 1 prior version. Project 152 (GEN) mapped to smoke folder 389645872290.

**Reconcile canary DONE + pushed (2026-06-13, commit aa0b3ab + scheduler 62b2903):** read-only daily canary, no write gate needed (validates setup pre-cutover). `BoxReconcileService.run()` = auth canary (CCG mint + /users/me; non-retryable fail → critical issue, transient → skip) + per-`[box].[ProjectFolder]` folder-visibility GET (404/403 → critical `folder_not_visible` = lost collaboration) + 25-row registry spot-check (`ReadRecentBoxFiles` → GET /files/{id}; 404 → high `registry_file_missing`). De-dups via `BoxReconciliationIssueService.open_drift_keys()` so persistent drift flags once. `POST /api/v1/admin/reconcile/box` (drain-secret), scheduler `reconcile_box` timer 05:00. `ReadRecentBoxFiles` sproc deployed to prod DB. Live-verified on test bed: healthy pass (0 issues), injected `registry_file_missing` (flagged high), dedup re-run (0 flagged).

**API DEPLOYED TO PROD 2026-06-15:** image `f89f2b7` (`az acr build` → digest c6456dae, pushed `:latest` + commit tag; container tag verified `:latest`; `az webapp restart`; prod root 200 + all 6 box routes confirmed in `/openapi.json`). Ships alongside the budget app layer (its DB was already live in prod). **Box ships DORMANT** — no `ALLOW_BOX_WRITES`, no `BOX_CLIENT_ID/SECRET/ENTERPRISE_ID` in prod App Service, so every box path no-ops. `.dockerignore` excludes `*.md`, so the other session's uncommitted `prompt.md` edits did NOT ship.

**Web UI DROPPED (Chris, 2026-06-13):** QBO + MS Graph have no web console; they're run via SQL + scripts + runbooks, and Box already matches (retry script, 3 runbooks, queryable tables, drain-secret endpoints). Folder mapping via `POST /box/map-project` or a future field on the Project edit page — not a standalone console. The UI-only read endpoints (outbox counts / dead-letter list / requeue / reconcile-issue reads) are NOT built.

**Remaining for Phase 2 close-out:** App Insights alerts (dead_letter>0, oldest-pending>10m, mint failures); **scheduler publish** — box timers `drain_box_outbox` (30s) + `reconcile_box` (daily) are committed+pushed (build.one.scheduler eb4561a/62b2903) but NOT yet `func azure functionapp publish`ed. Leave unpublished until box goes live for a real client tenant (publishing now = ~2880 no-op drain calls/day against a dormant/unconfigured box). Phase 3 (Excel) blocked on workbook copies from the client. **Box production cutover (future) =** authorize the app in the client's Box Admin Console → set BOX_CLIENT_ID/SECRET/ENTERPRISE_ID (client's enterprise) + collaborate the per-enterprise service account into folders → map projects → `func publish` scheduler → flip `ALLOW_BOX_WRITES=true`.

## Session: Budget entity Phase 2 — variance engine (2026-06-12)

Continuation of the 2026-06-11 session (same working tree, still uncommitted with Phase 1). Variance engine built, adversarially verified, deployed to prod DB, smoke-tested end-to-end.

**New artifacts:** `entities/budget/sql/dbo.budget_variance.sql` — `ReadBudgetVarianceByProjectId(@ProjectId, @BudgetId, @Actor…)` (SCC-grain budget/actual/drawn rows, single result set) + `ReadBudgetListRollups` (per-budget contract value + drawn). `scripts/migrations/budget_variance_support_indexes.sql` — IX_ContractLaborLineItem_ProjectId (INCLUDE variance columns; CLLI previously full-scanned), IX_EmployeeLaborLineItem_{ProjectId,EmployeeLaborId,PublicId} (ELLI shipped with ZERO nonclustered indexes); guards mirrored into both canonical DDL files. Python: `BudgetRepository.read_variance_by_project_id`/`read_list_rollups`, `BudgetService.variance_by_public_id` (rows + Python-Decimal CostCode subtotals + totals, all strings on the wire) / `read_all_with_rollups`, `GET /api/v1/get/budget/{public_id}/variance`, list endpoint now returns rollup fields.

**Verified predicates (encoded in the sproc header):** CL cost = `Price/NULLIF(1+ISNULL(Markup,0),0)` (Rate is DAILY — Hours×Rate is ~8× wrong; NULLIF guards a hand-entered −100% markup from bricking the endpoint); EL cost = `Hours×Rate` (Price is markup-inclusive); CL no-double-count = `BillLineItemId IS NULL OR IsBillable=0` (line-level FK, self-healing via ON DELETE SET NULL); drawn SCC resolved via `COALESCE(ili.SCC, bli.SCC, eli.SCC, bcli.SCC, elli.SCC)` branching on FK presence (QBO re-pull clobbers SourceType); drawn signs: BillCredit-sourced and expense-credit-sourced negative, Manual sign from ILI.Amount (Price stored positive); drafts count on all cost legs; drawn = `Invoice.IsDraft=0` strictly; IsOverhead excluded; `UnpricedLaborHours` per SCC keeps NULL-rate labor honest.

**Bug caught by live sanity check (not by any reviewer):** the Uncategorized bucket silently zeroed — aggregate CTEs joined on `SccId = SccId` and NULL=NULL is never true, vanishing $8.1M of QBO-pulled SCC-less draw lines on project 18. Fixed with a `-1` sentinel key (`ISNULL(scc,-1)` in every aggregate + `NULLIF(…,-1)` on output). After the fix, sproc drawn total matched an independent signed line-sum to the penny.

**Adversarial verification (3-agent panel):** empirical agent recomputed all 9 legs per SCC **independently from the policy text** across 4 projects (18/76/93/49 — 455 rows): ZERO mismatches at ±$0.02, RBAC scoping byte-identical for non-admin users, timings 0.15–0.74s. SQL + Python reviewers' findings all applied: div-by-zero guard, the two index migrations, `@BudgetId` passed by the service so an archived budget's variance can never report a different live budget's revisions, `ReadBudgets` now excludes archived (matching the rollup sproc — no fabricated $0 rollups), `Decimal("0.00")` accumulators, uuid guard on `read_by_public_id` (malformed id → 404 not 500), route-order comments corrected, variance access decision documented (Budgets can_read exposes cross-entity aggregates — deliberate, those are the cost-visibility roles).

**Data realities for Phase 3 (UI):** most drawn value lands in the Uncategorized bucket today (project 18: $8.6M of $9.2M) because QBO-pulled invoice lines carry no SCC — surface that row prominently. ~$6.0M of cost-side line value sits on NULL-ProjectId rows, invisible to every project's variance (policy blind spot, quantified; ops follow-up in TODO). EmployeeLaborLineItem is empty in prod — the EL leg is verified only at zero; re-verify when real W2 data lands.

**End-to-end smoke (project 93, cleaned up after):** budget created + 2 lines + activated → variance payload (69 SCC rows / 46 cost codes, budget legs flow into remaining/cost-variance per row) in ~2.1s; list rollup contract_value=85,000.00 matched; uncategorized subtotal matched the verifier's number to the penny; malformed public_id → None/404.

Files: entities/budget/sql/dbo.budget_variance.sql (new), scripts/migrations/budget_variance_support_indexes.sql (new), entities/budget/{persistence/repo.py, business/service.py, api/router.py, sql/dbo.budget.sql}, entities/contract_labor/sql/dbo.contract_labor.sql (index mirror), entities/employee_labor/sql/dbo.employee_labor.sql (index mirror). DB state: all deployed + index migration run. Code still uncommitted (combined Phase 1+2 commit pending).

## Session: Budget entity Phase 0 + Phase 1 (2026-06-11)

New Budget entity — customer-facing contract value per project (original + change-order deltas), full plan at umbrella memory `project_budget_entity.md`. Built via multi-agent workflow, adversarially reviewed (3 lenses) before deploy, lifecycle smoke-tested against the live DB.

**Schema (deployed to prod DB):** `dbo.Budget` (1 per project, Status draft→active→archived, filtered unique `UQ_Budget_ProjectId_Active WHERE Status <> 'archived'`), `dbo.BudgetRevision` (RevisionNumber atomic in Create sproc; Type original|change_order; Status draft→approved with ApprovedBy stamps), `dbo.BudgetLineItem` (BillLineItem-shaped: SubCostCodeId, Qty/Rate DECIMAL(18,4), Amount/Price DECIMAL(18,2), Markup; all NULLable for grid auto-save; negatives allowed for CO deltas). 21 sprocs. Contract value per SCC = SUM(approved revisions' lines).

**Lifecycle enforcement (service + SQL belt-and-suspenders):** approved revisions immutable — service pre-checks PLUS in-sproc UPDLOCK parent-status guards on all three line-item mutation sprocs (closes the mutate-racing-approval TOCTOU); `ActivateBudgetById` re-validates Rev-0 completeness in-txn and flips Budget + Rev 0 atomically; approve requires `row_version` in body; activation/approval refuse a missing actor (no COALESCE-to-17 on ApprovedByUserId — attribution must be real). Budget delete draft-only with explicit revision→line cascade (`force_internal` plumbing).

**Two pyodbc sproc conventions discovered the hard way (now in CLAUDE.md):** (1) mutation sprocs MUST `SET NOCOUNT ON` — bare DML before the final SELECT emits row-count tokens and `fetchone()` raises "No results"; (2) NEVER `ROLLBACK TRANSACTION` in-proc — pyodbc runs autocommit-off, the rollback zeroes the implicit outer txn → error 266. All budget sprocs use validate-first / always-COMMIT with empty-result-set (or COMMIT-then-RAISERROR) conflict signaling.

**Registration:** `Modules.BUDGETS`, SYNCHRONOUS_TASKS + PROCESS_REGISTRY ×3, app.py routers (18 routes), `seed.BudgetsModule.sql` + `005_grant_budgets_module.sql` (TA/Controller full+CanApprove, PM CRU, Reviewer/Auditor read; Owner mirrored via seed.owner_role.sql re-run — all run against prod). ReadBudgets actor-scoped via inline `UserCanAccessProject` (fail-closed); by-id paths assert via `shared/access.py`.

**Engine fix (all entities):** `InstantWorkflowHandler.execute` now re-raises `EntityNotAccessibleError` after failing the workflow, so ProcessEngine mutations map to the app-level 404 handler instead of a 400 that confirmed existence + leaked internal ids (previously affected the bill family too).

**Phase 0:** the stale duplicate ContractLaborLineItem CRUD sprocs in `add_contract_labor_line_item_bill_line_item_id.sql` were removed (re-running that file would have clobbered the live `UpdateContractLaborLineItemById` BillLineItemId guard + dropped `@IsOverhead`/`@CreatedByUserId`); `@CreatedByUserId` folded into `dbo.contract_labor.sql`'s `CreateContractLaborLineItem` so the canonical file is actually re-runnable. Live prod sproc verified already-guarded — no DB change needed.

**Smoke-tested live (all pass):** create + Rev-0 auto-create, duplicate-create rejection, incomplete-line activation rejection, stale-rowversion 409, activate (budget=active + rev0=approved + approver stamped), service-level AND raw-SQL-level approved-line mutation rejection, CO create/empty-approve-rejection/negative-delta-line/approve/double-approve-rejection, original-delete + active-budget-delete rejections. Smoke rows cleaned up.

**NOT done:** lifecycle unit tests (repo has no test infra — deviation from plan recorded in TODO), Phase 2 variance sproc/endpoints, web UI (Phase 3, desktop-layout route decision made), Excel write-back + agent/MCP tools (Phase 4, deferred). Code NOT yet committed/deployed — DB schema is live but API container doesn't carry the routes until `az acr build` + restart.

Files: entities/budget/**, entities/budget_revision/**, entities/budget_line_item/**, entities/module/sql/seed.BudgetsModule.sql, entities/role/sql/migrations/005_grant_budgets_module.sql, entities/contract_labor/sql/{dbo.contract_labor.sql, add_contract_labor_line_item_bill_line_item_id.sql}, core/workflow/business/{instant.py, definitions/instant.py}, shared/rbac_constants.py, app.py, CLAUDE.md.
...
**Executed night of 2026-06-10:** deleted 3 test logs (1097/1101/1102) from user 37's entry 774 (Chris's wrong-parent test artifacts — see build.one.ios SESSION_NOTES round 5), ran cleanup.time_entry_duplicates.sql @Apply=1 (all auto-mergeable groups resolved; remaining: conflict logs 88/90/751 + submitted dup days users 41/47 for office review), ran the guards migration (sproc NULL-guards verified live; unique-index gates correctly skipped pending the 5 groups — re-run after resolution), deployed 56aa54c via az acr build + restart, container tag verified :latest.
...
- `GET /api/v1/contract-labor/{public_id}/daily-summary` — per-employee per-day 8h allocation widget. (U-164 correction: the sproc was never broken in prod — assignment-only `SELECT @var = …` is fine on pyodbc; the real break is DML→SELECT + `fetchone()`.)
...
(U-164 correction, verified 2026-07-28: the premise that `ReadContractLaborDailySummary` was broken was wrong — assignment-only `SELECT @var = …` does not trip pyodbc on prod. The real pyodbc failure shape is DML followed by a row-returning SELECT with `fetchone()`. The standalone migration file [sql/ReadContractLaborDailySummary.sql](entities/contract_labor/sql/ReadContractLaborDailySummary.sql) added `SET NOCOUNT ON` under that mistaken premise; U-162 reconciled the base to live prod; U-164 added `SET NOCOUNT ON` + removed the read-only transaction as hygiene.)
...
- Rebake [entities/contract_labor/sql/dbo.contract_labor.sql](entities/contract_labor/sql/dbo.contract_labor.sql) as a consolidated snapshot of all post-migration sproc states (including `SET NOCOUNT ON` on `ReadContractLaborDailySummary` and the new `ReadContractLaborDistinctBillingPeriods`). Same pattern noted on May 7 — base files rot, migration files are authoritative. **[Superseded — U-162 made `dbo.contract_labor.sql` the single canonical source for all 28 CL sprocs (guarded by `tests/test_sproc_single_source.py`) and U-164 added the `SET NOCOUNT ON` pins; the "migration files are authoritative" guidance in this bullet is INVERTED relative to current policy — the base file is authoritative, migrations must not carry duplicate sproc bodies.]**
...
## 2026-09-02 — U-357 Phase 0 (`/em`): U-357a census · U-357b review base reconcile · U-357d api error_code
- **U-357a** (read-only prod census, results in design doc §3a): ContractLabor still on the legacy vocab (billed 1,128 / submitted 38 / pending_review 23 / ready 5; 0 stray values — the 2026-07-02 rename never ran); EmployeeLabor 0 rows; TimeEntry latest-row approved 859 / billed 260 (a Feb–Mar one-time backfill, no writer in code) / draft 10 / submitted 7; Review rows Bill 751 · Invoice 49 · ContractLabor 508 · Expense 0 · BillCredit 0; Bill has 65 finalized-with-open-review rows; `IsDraft` has zero dependents on the four documents (system-named DEFAULTs only); live `dbo.Review` is the 5-parent shape.
- **U-357b** (P0-surface, built directly, Codex xhigh r2 PASS): `entities/review/sql/dbo.review.sql` reconciled to live — `ContractLaborId` in CREATE TABLE + guarded ALTER block (column/FK/filtered IX/5-way `CK_Review_OneParent`), `vw_Review` + `CreateReview @ContractLaborId = NULL` = live bodies; migrations 003/005 and `scripts/migrations/gap2_adjacent_threading.sql` §7 (a stale `CreateReview` lacking `@ContractLaborId`/`@EmailMessageId` — a live U-037-class hazard) stubbed; `("review", REVIEW_BASE)` whole-file guard row; `CreateReview` ledger row deleted; digest `OUTER APPLY` Id tiebreak. Base==live parity proven for the three review objects → apply is a no-op except the digest sproc. Process note: the isolated `/private/tmp` worktree was wiped mid-unit after Codex r2 PASS; rebuilt from the transcript's replacement scripts (see umbrella memory `feedback_tmp_worktree_wiped_commit_wip_early`).
- **U-357d api** (Composer-built, Codex high r1 CR → r2 PASS → Pass 2 → r3 one P2 fixed): additive `error_code` SIBLING of `detail` on every error body (`shared/api/errors.py`: `ErrorCode` constants = wire contract, `error_response()` single builder, handlers on Starlette `HTTPException` + `RequestValidationError` + the `EntityNotAccessibleError` 404 masker); `raise_workflow_error` derives `transition_invalid`/`entry_locked` from the phrases the server emits (bridge, retire in LS-01); `raise_database_error` → `duplicate_key`/`fk_violation`; `raise_not_found` → `not_found`; `X-Client-Build` read folded into `ProxyHeadersMiddleware` (one layer). `detail` text + status codes byte-identical (iOS substring-matches them). Suite 3032 green; both regression tests mutation-proven RED.
