...
IF OBJECT_ID('dbo.Vendor', 'U') IS NOT NULL
   AND NOT EXISTS (SELECT 1 FROM sys.columns
                   WHERE object_id = OBJECT_ID('dbo.Vendor') AND name = 'CreatedByUserId')
BEGIN
    ALTER TABLE [dbo].[Vendor] ADD [CreatedByUserId] BIGINT NOT NULL
        CONSTRAINT [DF_Vendor_CreatedByUserId] DEFAULT (17);
END
GO
IF OBJECT_ID('dbo.Vendor', 'U') IS NOT NULL
   AND NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = 'FK_Vendor_CreatedByUser')
BEGIN
    ALTER TABLE [dbo].[Vendor] ADD CONSTRAINT [FK_Vendor_CreatedByUser]
