...
# Local Imports
from core.ai.agents.expense_agent.business.models import (
    FilenameParsedData,
    ProcessingResult,
)
...
def parse_expense_filename(filename: str) -> FilenameParsedData:
    """
    Parse an expense filename using the 8-segment convention:
    {Cardholder} - {Project} - {Vendor} - {ExpenseNumber} - {Description} - {SubCostCode} - {Rate} - {ExpenseDate}

    Examples:
        John Doe - HP2 - Teran Remodeling - EXP001 - Office Supplies - 18.1 - $50.00 - 3-6-2026.pdf
    """
    ...
    data.cardholder = segments[0].strip() or None
    data.project_abbrev = segments[1].strip() or None
    data.vendor_name = segments[2].strip() or None
    data.expense_number = segments[3].strip() or None
    data.description = segments[4].strip() or None
    data.sub_cost_code_raw = segments[5].strip() or None

    # Parse rate: strip $ and commas
    rate_str = segments[6].strip().replace('$', '').replace(',', '')
    if rate_str:
        try:
...
class ExpenseFolderProcessor:
    """
    Deterministic processing service for expense folder files.
    Lists files from SharePoint source folder, extracts data from each PDF,
    creates expense drafts, and moves processed files to the processed folder.
    """
    ...
    def process(self, company_id: int, tenant_id: int = 1, on_progress=None) -> ProcessingResult:
        ...
        3. For each file: parse filename, OCR, extract, create expense draft, move file
        """
        ...
        if not pdf_files:
            logger.info("No PDF files found in expense source folder")
            return result
    ...
    def _process_single_file(
        ...
        """Process a single PDF file: extract data, create expense draft, move file."""
        filename = file_item.get("name", "")
        file_item_id = file_item.get("item_id", "")

        logger.info("Processing expense file: %s", filename)
        ...
        # Step 3: OCR extraction (skip if filename gave us all key fields)
        filename_complete = all([
            parsed.vendor_public_id, parsed.expense_number, parsed.expense_date,
        ...
        expense_number = parsed.expense_number
        ...
        memo = parsed.cardholder  # Cardholder stored in memo
        ...
        if extraction_result:
            ...
            if not expense_number and extraction_result.bill_number:
                expense_number = extraction_result.bill_number
            if not expense_date and extraction_result.bill_date:
                expense_date = extraction_result.bill_date
            if extraction_result.total_amount and not total_amount:
                total_amount = extraction_result.total_amount
            if not description and extraction_result.memo:
                description = extraction_result.memo
        ...
        if not expense_number:
            raise ValueError(f"No expense number found for '{filename}'")
        if not expense_date:
            raise ValueError(f"No expense date found for '{filename}'")

        # Step 6b: Check for duplicate expense (same reference_number + vendor)
        existing_expense = self.expense_service.read_by_reference_number_and_vendor_public_id(
            reference_number=expense_number, vendor_public_id=vendor_public_id,
        )
        if existing_expense:
            logger.info(
                "Expense already exists for vendor=%s, reference_number=%s — skipping creation, moving file: %s",
                vendor_public_id, expense_number, filename,
            )
            try:
        ...
        file_extension = ".pdf"
        blob_name = f"expenses/{uuid.uuid4()}{file_extension}"
        ...
        # Step 9: Create ExpenseLineItem (primary from filename)
        ...
        # Step 10: Link attachment to expense line item
        ...
        try:
            ...
            logger.warning("Failed to move '%s' to processed folder: %s (expense was still created)", filename, e)

        result.files_processed += 1
        result.expenses_created += 1
        logger.info(
            "Created expense draft: vendor=%s, reference_number=%s, amount=%s, file=%s",
            vendor_public_id, expense_number, total_amount, filename,
        )
