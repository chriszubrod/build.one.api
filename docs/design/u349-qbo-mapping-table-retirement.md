...
| 11 | expense_line_item (PurchaseLineExpenseLineItem) | U-364 | line-item | **1** | reconciliation + purchase svc/router | hard (recon) | ✅ done+deployed+dropped 2026-09-21 (bc32e384 / ACR cabt, image sha256:12f846ac; table 12,177 rows dropped + 5 CRUD sprocs; readers repointed by U-494/U-491/U-492, registry pruned by U-493a, review round U-495). Census 0 strand risk; 0 incoming FKs; pre-drop dump captured (docs/operations/u493_*). Post-drop: both silent paths assert rows (detector 33, coding-state 5), and the OBJECT_ID-guarded DeleteExpenseCascadeById bridge verified to no-op rather than throw. **ALL 11 FAMILIES RETIRED.** |
...
Net at completion: `qbo.*` **20** — DONE, measured live after the drop 2026-09-21. (The original "30 → 19" was an estimate made before any family landed and never held: 10 of the 11 mapping tables are already gone and the live count is 21, so family 11's DROP takes it to 20.) Separately, the ~15 raw-staging entity tables
