...
from agents.definitions.base import WorkflowDefinition
from agents.definitions.bill_intake import (
    BILL_INTAKE_WORKFLOW,
    EMAIL_INTAKE_WORKFLOW,
    BILL_PROCESSING_WORKFLOW,
)
from agents.exceptions import WorkflowStepError
from agents.models import Workflow
...
from agents.runners.base import AgentContext, AgentResult
...
from agents.runners.bill_extraction import BillExtractionAgent
...
class BillIntakeExecutor:
    ...
    def __init__(
        ...
        # Initialize agents
        self.email_triage_agent = EmailTriageAgent(self.capabilities)
        self.approval_parser_agent = ApprovalParserAgent(self.capabilities)
        self.correlation_agent = CorrelationAgent(self.capabilities)
        self.bill_extraction_agent = BillExtractionAgent(self.capabilities)
    ...
    async def run_triage(
        ...
        result = await self.email_triage_agent.run(context)
    ...
    async def process_approval_reply(
        ...
        result = await self.approval_parser_agent.run(context)
    ...
    async def _run_bill_extraction(
        ...
        result = await self.bill_extraction_agent.run(context)
    ...
    async def _run_vendor_matching(self, workflow: Workflow) -> Workflow:
        ...
        # Try to match vendor
        match_result = self.capabilities.entity.match_vendor(
            name=vendor_name,
            tenant_id=workflow.tenant_id,
        )
        ...
        if match_result.success and match_result.data:
            ...
            if isinstance(data, dict):
                ...
                vendor_match = {
                    "matched": True,
                    "vendor": data,
                    "match_type": match_result.metadata.get("match_type"),
                    "confidence": match_result.metadata.get("confidence", 1.0),
                }
                trigger = "vendor_matched"
            elif isinstance(data, list) and len(data) > 0:
                # Fuzzy match candidates
                top_candidate = data[0]
                if hasattr(top_candidate, 'confidence') and top_candidate.confidence >= 0.85:
                    # High confidence fuzzy match - auto-accept
                    vendor_match = {
                        "matched": True,
                        "vendor": {
                            "id": top_candidate.id,
                            "public_id": top_candidate.public_id,
                    ...
                    # Low confidence - provide candidates but mark as not matched
                    vendor_match = {
                        "matched": False,
                        "candidates": [
                            {
                ...
                vendor_match = {"matched": False, "reason": "No matching vendors found"}
                trigger = "vendor_not_found"
        else:
            vendor_match = {"matched": False, "reason": match_result.error or "Match failed"}
            trigger = "vendor_not_found"
