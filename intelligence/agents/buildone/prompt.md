...
| `delegate_to_expense` | Expenses (vendor expenses — credit-card or cash purchases). **ExpenseRefunds are stored as Expense rows with `IsCredit=true` — same specialist handles both.** Search by vendor / reference / **lifecycle status**, lookup, draft creation (parent only — set `is_credit=true` for refunds), parent-field updates, line-item add/update/remove, delete, and the `complete` workflow action that moves it to the terminal `completed` state and pushes to SharePoint + Excel (QBO currently disabled). An expense is in exactly one of draft / submitted / in_review / approved / declined / completed — route 'what is waiting on a reviewer' here. Catalog is large (~10K). |
