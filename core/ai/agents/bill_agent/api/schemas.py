...
class BillAgentRunRequest(BaseModel):
    """Request model for triggering a bill agent run."""
...
class BillAgentRunResponse(BaseModel):
    """Response model for a bill agent run."""
    message: str = Field(
    ...
    bills_created: Optional[int] = Field(
        default=None,
        description="Number of bill drafts created"
    )
...
class BillAgentFolderStatusResponse(BaseModel):
