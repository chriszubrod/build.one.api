...
class MapBillIntuitBill:
    ...
    @classmethod
    def from_db_row(cls, row) -> 'MapBillIntuitBill':
        """Creates a MapBillIntuitBill object from a database row."""
        if not row:
            return None
...
def create_map_bill_intuit_bill(
        bill_id: int,
        intuit_bill_id: int
    ) -> PersistenceResponse:
    """
    ...
    with pers_database.get_db_connection() as cnxn:
        try:
            with cnxn.cursor() as cursor:
                ...
                if rows:
                    return PersistenceResponse(
                        data=[MapBillIntuitBill.from_db_row(row) for row in rows],
    ...
    with pers_database.get_db_connection() as cnxn:
        try:
            with cnxn.cursor() as cursor:
                ...
                if row:
                    return PersistenceResponse(
                        data=MapBillIntuitBill.from_db_row(row),
    ...
    with pers_database.get_db_connection() as cnxn:
        try:
            with cnxn.cursor() as cursor:
                ...
                rowcount = cursor.execute(
                    sql,
                    map_bill_intuit_bill.id,
                    map_bill_intuit_bill.bill_id,
                    map_bill_intuit_bill.intuit_bill_id
                ).rowcount
                cnxn.commit()
