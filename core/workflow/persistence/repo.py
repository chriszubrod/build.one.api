...
class WorkflowRepository:
    ...
    def get_all_conversation_ids(self) -> set[str]:
        ...
        try:
            with get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("SELECT DISTINCT ConversationId FROM dbo.Workflow WHERE ConversationId IS NOT NULL")
                rows = cursor.fetchall()
    ...
    def read_by_tenant_and_type(
        ...
        """
        Read workflows by tenant and workflow type.
        
        This queries directly without the parent/child filtering used in read_by_tenant_and_state.
        """
        try:
            with get_connection() as conn:
                cursor = conn.cursor()
                if state:
                    cursor.execute(
                        "SELECT * FROM dbo.Workflow WHERE TenantId = ? AND WorkflowType = ? AND State = ? ORDER BY CreatedDatetime DESC",
                        [tenant_id, workflow_type, state]
                    )
                else:
                    cursor.execute(
                        "SELECT * FROM dbo.Workflow WHERE TenantId = ? AND WorkflowType = ? ORDER BY CreatedDatetime DESC",
                        [tenant_id, workflow_type]
                    )
                rows = cursor.fetchall()
                return [self._from_db(row) for row in rows if row]
        except Exception as error:
            logger.error("Error during read Workflows by tenant and type: %s", error)
            raise map_database_error(error)
