...
>
> * **LS-03c Expense — BUILT as U-467 (2026-09-16).** One slice straight to the end state
>   (U-445 + U-446 together): `Status`/`StatusDatetime`/`StatusOrigin`/`StatusSourceRef` +
>   `CK_Expense_Status` / `CK_Expense_StatusOrigin`, in-file `CK_Expense_Status_IsDraft`
>   (created then dropped — the apply-time parity proof), filtered `IX_Expense_Status`,
>   set-based backfill, `IsDraft` as `PERSISTED NOT NULL` computed over Status,
>   `FinalizeExpenseById` (replaces the `is_draft=False` PUT in `complete_expense`),
>   `TransitionExpenseStatus` (sproc only, U-446c reopen fence), `@Status` on
>   list+count, U-447 single-snapshot `ReadExpensesPaginated`, `?status=` / `?start_date=` /
>   `?end_date=` on `GET /get/expenses`, `CreateReview` Expense mirror, Purchase connector
>   CREATE binds `status='completed'` / `status_origin='qbo_pull'` /
>   `status_source_ref='qbo:<realm>/<id>'`. **`dbo.expense.sql` and `dbo.review.sql` must be
>   applied in ONE transaction** (schema first leaves a window where Expense stores Status
>   but CreateReview does not mirror it; review first fails with 207; old CreateExpense
>   against a computed IsDraft is SQL error 271).
>
>   **Backfill census (prod, read-only, 2026-09-16):** 11,753 Expense rows; 11,743
>   `IsDraft=0` (ALL have QboId and RealmId) → `completed`/`qbo_pull`; 10 `IsDraft=1`
>   (no QboId) → `draft`/`backfill`. `dbo.Review` has 0 Expense rows; `dbo.CompletionJob`
>   has 0 Expense rows (the completion-origin branch is still written).
>   `MAX(LEN(CONCAT('qbo:',RealmId,'/',QboId)))` = 26. Terminal lock, delete cascade,
>   agent vocabulary and web layout are U-468 / U-469 / U-470.
>
...
**Expense IsDraft writer sweep (U-467, 2026-09-16).** Three physical writers: `CreateExpense`
INSERT (removed from the column list — error 271 otherwise), `UpdateExpenseById` SET
(removed; compat translation writes Status only), and `scripts/verify_propose_invoice_source_links_tier0.py`
`new_expense()` (now INSERTs `Status='completed'`). Service call sites: `create` ×3
(the parameter is KEPT — tests/test_u458_completion_gate.py pins it — but the Purchase
connector no longer passes `is_draft`; it passes the status triple), `complete_expense` ×1
(replaced by `FinalizeExpenseById`). `ExpenseLineItem.IsDraft` is a real, separate column
and was not swept.
