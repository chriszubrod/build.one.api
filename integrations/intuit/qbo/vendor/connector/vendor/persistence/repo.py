...
class VendorVendorRepository:
    ...
    def read_identity_check(self, *, local_id: int, qbo_id: str) -> IdentityCheckResult:
        """
        Single JOIN'd read backing `verify_vendor_qbo_identity` (U-306):
        replaces the old `read_by_vendor_id` + `qbo_vendor_repo.read_by_id`
        2-read path with one round trip, and additionally resolves whether
        the mapping table already binds `qbo_id` to a DIFFERENT VendorId
        ...
        try:
            with get_connection() as conn:
                ...
                try:
                    call_procedure(
                        cursor=cursor,
                        name="ReadVendorVendorIdentityCheckByVendorId",
                        params={"VendorId": local_id, "QboId": qbo_id},
                    )
                    row = cursor.fetchone()
            ...
            logger.error(f"Error during read vendor vendor identity check: {error}")
            raise map_database_error(error)
