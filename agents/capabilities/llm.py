...
class LlmCapabilities(Capability):
    """
    LLM-based capabilities using Azure OpenAI.
...
Respond in JSON format:
{
    ...
    "reasoning": "brief explanation of why this entity type was chosen"
            ...
            # chat_completion_with_json returns parsed JSON directly or raises exception
            content = client.chat_completion_with_json(messages)
            ...
            result = CapabilityResult.ok(data=classification)
            self._log_result("classify_email", result)
            return result
            
        except Exception as e:
            logger.exception("Error in classify_email")
...
Respond in JSON:
{
    ...
    "question_text": "string or null if decision is question"
            ...
            response = client.chat_completion_with_json(messages)
            
            if response.get("status_code") != 200:
                return CapabilityResult.fail(
                    error=response.get("message", "LLM call failed"),
            ...
            logger.exception("Error in parse_approval_reply")
            return CapabilityResult.fail(error=str(e))
...
Respond in JSON:
{
    ...
    "reasoning": "why this matches or doesn't match"
            ...
            logger.exception("Error in match_orphan_email")
            return CapabilityResult.fail(error=str(e))
...
Respond in JSON format:
{
    ...
    "extraction_notes": "any issues or uncertainties"
            ...
            logger.exception("Error in extract_bill_fields")
            return CapabilityResult.fail(error=str(e))
    ...
    def simple_completion(
        ...
        try:
            ...
            logger.exception("Error in simple_completion")
            return CapabilityResult.fail(error=str(e))
