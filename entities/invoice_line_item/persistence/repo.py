...
class InvoiceLineItemRepository:
    ...
    def _from_db(self, row: pyodbc.Row) -> Optional[InvoiceLineItem]:
        ...
        try:
            return InvoiceLineItem(
                ...
                employee_labor_line_item_id=getattr(row, "EmployeeLaborLineItemId", None),
            ...
            with get_connection() as conn:
                ...
                call_procedure(
                    ...
                    params={
                        ...
                        "EmployeeLaborLineItemId": employee_labor_line_item_id,
        ...
        try:
            with get_connection() as conn:
                ...
                params = {
                    ...
                    "EmployeeLaborLineItemId": getattr(line_item, "employee_labor_line_item_id", None),
    ...
    def delete_by_bill_line_item_id(self, bill_line_item_id: int) -> None:
        try:
            with get_connection() as conn:
                cursor = conn.cursor()
                call_procedure(cursor=cursor, name="DeleteInvoiceLineItemsByBillLineItemId", params={"BillLineItemId": bill_line_item_id})
        except Exception as error:
            logger.error(f"Error deleting invoice line items for bill_line_item_id={bill_line_item_id}: {error}")
            raise map_database_error(error)
        ...
        *,
        ...
        try:
            with get_connection() as conn:
                ...
                call_procedure(
                    cursor=cursor,
                    name="SetInvoiceLineItemQboIdentity",
                    params={
                        "Id": id,
                        "QboId": qbo_id,
                        "RealmId": realm_id,
                ...
                if row and getattr(row, "Stolen", False):
                    logger.warning(
                        "InvoiceLineItem %s stole QBO identity (qbo_id=%s realm_id=%s) from a "
                        "different InvoiceLineItem row under the same Invoice — a stale duplicate "
                        "identity existed before this stamp",
                        id, qbo_id, realm_id,
                    )
        except Exception as error:
            logger.error(
                "Error stamping InvoiceLineItem QBO identity (invoice_line_item_id=%s qbo_id=%s realm_id=%s): %s",
                id,
                qbo_id,
                realm_id,
                error,
