...
- [ ] **Wave E — remaining gaps + shared infra:** fix React gaps for admin, auth (password reset — if shipped alongside). Then delete `get_current_user_web`, `require_module_web`, `WebAuthenticationRequired` + its `app.py` handler, CSRF helpers, `entities/auth/web/controller.py`, the 5 QBO `web/controller.py` files, and the `templates/` tree.
