...
| [deploy-stale-image-after-restart.md](deploy-stale-image-after-restart.md) | `az webapp restart` relaunches the CACHED image and reports success while OLD code keeps serving — even with a correct `:latest` tag; fix is `stop`+`start`, and verify with a behavioral sentinel not a 200 |
