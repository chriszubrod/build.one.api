...
@api_module_bp.route('/patch/module', methods=['PATCH'])
def api_patch_module_route():
    """
    Handles the PATCH request for updating a module.
    """
    try:
        # If request is not JSON, return 400 error
        if not request.is_json:
        ...
        # Get the module guid, strip whitespace, clean with bleach and html.escape
        raw_module_guid = data.get('guid', '').strip()
        clean_module_guid = bleach.clean(raw_module_guid, strip=True)
        module_guid = html.escape(clean_module_guid)
        ...
        # Call the post_module function and pass in the data to create a module
        module_bus_response = bus_module.patch_module_by_guid(
            guid=module_guid,
            modified_datetime=submission_datetime,
            name=module_name,
            description=module_description,
        ...
        # Return the response from the patch_module function
        return jsonify(
            ApiResponse(
                data=module_bus_response.data,
                message=module_bus_response.message,
