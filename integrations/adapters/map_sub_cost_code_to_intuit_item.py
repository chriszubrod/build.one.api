...
@register_adapter
@dataclass
class MapSubCostCodeToIntuitItem:
    ...
    @classmethod
    def from_db_row(cls, row) -> 'MapSubCostCodeToIntuitItem':
        """Creates a MapSubCostCodeToIntuitItem object from a database row."""
        if not row:
            return None
...
def create_map_sub_cost_code_to_intuit_item(
...
def read_map_sub_cost_code_to_intuit_items() -> PersistenceResponse:
    """
    ...
    with get_db_connection() as cnxn:
        try:
            with cnxn.cursor() as cursor:
                ...
                if rows:
                    return PersistenceResponse(
                        data=[MapSubCostCodeToIntuitItem.from_db_row(row) for row in rows],
...
def read_map_sub_cost_code_to_intuit_item_by_sub_cost_code_id(sub_cost_code_id: int) -> PersistenceResponse:
    with get_db_connection() as cnxn:
        try:
            with cnxn.cursor() as cursor:
                ...
                if row:
                    return PersistenceResponse(
                        data=MapSubCostCodeToIntuitItem.from_db_row(row),
...
def read_map_sub_cost_code_to_intuit_item_by_intuit_item_id(intuit_item_id: int) -> PersistenceResponse:
    with get_db_connection() as cnxn:
        try:
            with cnxn.cursor() as cursor:
...
def update_map_sub_cost_code_to_intuit_item(map_sub_cost_code_to_intuit_item):
    with get_db_connection() as cnxn:
        try:
            with cnxn.cursor() as cursor:
                ...
                rowcount = cursor.execute(
                    sql,
                    map_sub_cost_code_to_intuit_item.id,
                    map_sub_cost_code_to_intuit_item.sub_cost_code_id,
                    map_sub_cost_code_to_intuit_item.intuit_item_id
                ).rowcount
                cnxn.commit()
