...
@dataclass
class MapProjectIntuitCustomer:
    ...
    @classmethod
    def from_db_row(cls, row) -> Optional['MapProjectIntuitCustomer']:
        if not row:
            return None
        return cls(
...
def create_map_project_intuit_customer(project_id: int, intuit_customer_id: int) -> PersistenceResponse:
...
def read_map_project_intuit_customers() -> PersistenceResponse:
    with get_db_connection() as cnxn:
        try:
            with cnxn.cursor() as cursor:
                ...
                if rows:
                    return PersistenceResponse(
                        data=[MapProjectIntuitCustomer.from_db_row(r) for r in rows],
...
def read_map_project_intuit_customer_by_guid(guid: str) -> PersistenceResponse:
    with get_db_connection() as cnxn:
        try:
            with cnxn.cursor() as cursor:
                sql = "{CALL ReadMapProjectIntuitCustomerByGUID (?)}"
                row = cursor.execute(sql, guid).fetchone()
                if row:
                    return PersistenceResponse(
                        data=MapProjectIntuitCustomer.from_db_row(row),
...
def read_map_project_intuit_customer_by_project_id(project_id: int) -> PersistenceResponse:
    with get_db_connection() as cnxn:
        try:
            with cnxn.cursor() as cursor:
...
def update_map_project_intuit_customer(mapping: MapProjectIntuitCustomer) -> PersistenceResponse:
    with get_db_connection() as cnxn:
        try:
            with cnxn.cursor() as cursor:
...
def delete_map_project_intuit_customer_by_id(id: int) -> PersistenceResponse:
    with get_db_connection() as cnxn:
        try:
            with cnxn.cursor() as cursor:
