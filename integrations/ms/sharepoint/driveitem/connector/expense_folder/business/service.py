...
from integrations.ms.sharepoint.driveitem.connector.expense_folder.persistence.repo import DriveItemExpenseFolderRepository
...
class DriveItemExpenseFolderConnector:
    """
    Connector service for linking DriveItems (folders) to the Expense module at the company level.
    Supports two folder types: 'source' (intake) and 'processed' (completed).
    """

    def __init__(
        self,
        mapping_repo: Optional[DriveItemExpenseFolderRepository] = None,
        ...
        """Initialize the DriveItemExpenseFolderConnector."""
        self.mapping_repo = mapping_repo or DriveItemExpenseFolderRepository()
    ...
    def link_folder(
        ...
        """
        Link a DriveItem (folder) to the expense module for a company.
        ...
        if item_type != "folder":
            return {
                "message": "Only folders can be linked as expense processing folders",
                "status_code": 400,
                "mapping": None,
            }
        ...
        try:
            ...
            logger.info("Created expense folder mapping: Company %s, Type '%s' <-> DriveItem %s", company_id, folder_type, ms_driveitem_id)
            return {
            ...
            logger.exception("Error creating driveitem-expense-folder mapping")
            return {
                "message": str(e),
                "status_code": 500,
                "mapping": None,
    ...
    def get_folder(self, company_id: int, folder_type: str) -> Optional[dict]:
        """
        Get the linked driveitem details for an expense folder.
    ...
    def unlink_folder(self, company_id: int, folder_type: str) -> dict:
        """
        Unlink the driveitem from the expense module. Only removes the mapping.
        ...
        try:
            ...
            logger.info("Unlinked '%s' expense folder from company %s", folder_type, company_id)
            return {
            ...
            logger.exception("Error unlinking expense folder from company")
            return {
                "message": str(e),
                "status_code": 500,
                "mapping": None,
