...
- [ ] **Add an `upload_attachment` tool to `bill_specialist`** (and likely `expense_specialist` / `bill_credit_specialist` if their entities adopt the same rule). Once `BillService.create` rejects calls without an `attachment_public_id`, the existing agent `create_bill` tool will fail on every invocation. The fix is to give agents a way to upload a PDF (from a URL, SharePoint reference, or message attachment) so they can satisfy the rule. Ship before announcing the universal rule to anyone using the agent flow, OR ship the rule first and accept that agent-driven bill creation is broken in the interim. Track the affected period.
