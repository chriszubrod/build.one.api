        ...
        try:
            with get_connection() as conn:
                ...
                if row and getattr(row, "Stolen", False):
                    logger.warning(
                        "Bill %s stole QBO identity (qbo_id=%s realm_id=%s) from a different "
                        "Bill row — a stale duplicate identity existed before this stamp",
                        id, qbo_id, realm_id,
                    )
        except Exception as error:
            logger.error(
                "Error stamping Bill QBO identity (bill_id=%s qbo_id=%s realm_id=%s): %s",
                id,
                qbo_id,
                realm_id,
                error,
