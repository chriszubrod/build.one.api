...
-- entities/user_company/sql/migrations/001_phase0_access_control.sql. No-op against the live
-- schema (columns/FKs already exist there).
IF OBJECT_ID('dbo.UserCompany', 'U') IS NOT NULL
   AND NOT EXISTS (SELECT 1 FROM sys.columns
                   WHERE object_id = OBJECT_ID('dbo.UserCompany') AND name = 'CreatedByUserId')
BEGIN
...
IF OBJECT_ID('dbo.UserCompany', 'U') IS NOT NULL
   AND NOT EXISTS (SELECT 1 FROM sys.columns
                   WHERE object_id = OBJECT_ID('dbo.UserCompany') AND name = 'ModifiedByUserId')
BEGIN
    ALTER TABLE [dbo].[UserCompany] ADD [ModifiedByUserId] BIGINT NULL;
END
GO
IF OBJECT_ID('dbo.UserCompany', 'U') IS NOT NULL
   AND NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = 'FK_UserCompany_CreatedByUser')
BEGIN
...
GO
IF OBJECT_ID('dbo.UserCompany', 'U') IS NOT NULL
   AND NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = 'FK_UserCompany_ModifiedByUser')
BEGIN
