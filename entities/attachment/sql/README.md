...
table bootstrap and all 12 of its stored procedures. No migration may redefine
...
The 12 sprocs: `CreateAttachment`, `ReadAttachments`, `ReadAttachmentById`,
...
`IncrementDownloadCount`, `SetAttachmentQboIdentity`,
`ReadAttachmentByQboIdAndRealmId` (U-279).
...
categorization, attribution, and QBO-identity columns; prod already has
`ExtractionStatus` (+3 more, `add_extraction_fields.sql`), the `AICategory`
family (`add_categorization_fields.sql`), `CreatedByUserId`
(`scripts/migrations/gap2_created_by_user_id.sql`), and `QboId`/`RealmId` +
`UQ_Attachment_QboId_RealmId` (`scripts/migrations/238c_qbo_identity_reference.sql`,
U-238c). Re-applying only the sproc section is the usual path once bodies are
...
1. **`entities/attachment/sql/dbo.attachment.sql` (first pass)** — the guarded
   ...
   `AICategory` family, `CreatedByUserId`, and `QboId`/`RealmId` are not
   present yet. Continue — the second pass (step 6) applies them.
...
5. **`scripts/migrations/238c_qbo_identity_reference.sql`** — `QboId`/`RealmId`
   columns + `UQ_Attachment_QboId_RealmId` unique index on `dbo.Attachment`
   (and several other entities' tables in the same file).

6. **`entities/attachment/sql/dbo.attachment.sql` (second pass)** — idempotent
   `CREATE OR ALTER`; all 12 sprocs apply with the full column set.
