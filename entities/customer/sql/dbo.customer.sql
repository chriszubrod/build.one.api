...
IF OBJECT_ID('dbo.Customer', 'U') IS NOT NULL
   AND NOT EXISTS (SELECT 1 FROM sys.columns
                   WHERE object_id = OBJECT_ID('dbo.Customer') AND name = 'CreatedByUserId')
BEGIN
    ALTER TABLE [dbo].[Customer] ADD [CreatedByUserId] BIGINT NOT NULL
        CONSTRAINT [DF_Customer_CreatedByUserId] DEFAULT (17);
END
GO
IF OBJECT_ID('dbo.Customer', 'U') IS NOT NULL
   AND NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = 'FK_Customer_CreatedByUser')
BEGIN
    ALTER TABLE [dbo].[Customer] ADD CONSTRAINT [FK_Customer_CreatedByUser]
