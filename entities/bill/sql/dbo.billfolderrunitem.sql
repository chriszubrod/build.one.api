...
--
-- Run: ./.venv/bin/python scripts/run_sql.py entities/bill/sql/dbo.billfolderrunitem.sql
...
IF OBJECT_ID('dbo.BillFolderRunItem', 'U') IS NOT NULL
   AND NOT EXISTS (SELECT 1 FROM sys.columns
                   WHERE object_id = OBJECT_ID('dbo.BillFolderRunItem') AND name = 'CreatedByUserId')
BEGIN
    ALTER TABLE [dbo].[BillFolderRunItem] ADD [CreatedByUserId] BIGINT NOT NULL
        CONSTRAINT [DF_BillFolderRunItem_CreatedByUserId] DEFAULT (17);
END
GO
IF OBJECT_ID('dbo.BillFolderRunItem', 'U') IS NOT NULL
   AND NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = 'FK_BillFolderRunItem_CreatedByUser')
BEGIN
    ALTER TABLE [dbo].[BillFolderRunItem] ADD CONSTRAINT [FK_BillFolderRunItem_CreatedByUser]
