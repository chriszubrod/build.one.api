...
class QboCustomerClient:
    """
    Lightweight client for interacting with QBO Customer endpoints.
    """
        ...
        self._client.headers.update(
            {
                ...
                "User-Agent": "build.one-qbo-customer-client/1.0",
            }
        )
    ...
    def get_customer(self, customer_id: str) -> QboCustomer:
        ...
        Returns:
            QboCustomer: The customer information
        """
    ...
    def query_customers(
        ...
        Returns:
            List[QboCustomer]: List of customers matching the query
        """
        path = "/query"
        ...
        if last_updated_time:
            ...
            query_string = f"SELECT * FROM Customer WHERE Metadata.LastUpdatedTime > '{formatted_time}' STARTPOSITION {start_position} MAXRESULTS {max_results}"
            logger.debug(f"Querying Customers with WHERE clause: Metadata.LastUpdatedTime > '{formatted_time}'")
        else:
            query_string = f"SELECT * FROM Customer STARTPOSITION {start_position} MAXRESULTS {max_results}"
    ...
    def query_all_customers(self, last_updated_time: Optional[str] = None) -> List[QboCustomer]:
        ...
        Returns:
            List[QboCustomer]: List of all customers matching the query
        """
        all_customers = []
        start_position = 1
        max_results = 1000
        ...
        while True:
            ...
            # If we got fewer customers than requested, we're done
