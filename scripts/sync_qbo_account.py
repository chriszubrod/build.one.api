...
def run_locked(skip_sync_record_update: bool = False, dry_run: bool = False) -> dict:
    ...
    this function, so its own lock hold is untouched here. Locking must live
    at this outer layer, not inside `sync_qbo_account()` / `sync_qbo_to_local()`:
    those are shared with the admin path, which already holds this same
    resource for the duration of its call — a second acquire from inside
    that call would self-deadlock against itself.
    """
