...
U-347: the route now acquires the lock via the shared
`@qbo_sync_locked_route("account")` decorator (`integrations/intuit/qbo/
base/locking.py`), which internally calls `qbo_sync_lock(entity)` ->
`qbo_app_lock(qbo_entity_sync_lock_resource(entity))`. That is the ONE call
site left in the whole codebase that turns an entity name into a
`qbo_sync:<entity>` resource string, so these tests patch `qbo_app_lock`
there (`integrations.intuit.qbo.base.locking`), not on the router module —
the router no longer imports `qbo_app_lock` at all.
...
`service.sync_from_qbo` directly, without the `@qbo_sync_locked_route(...)`
decorator, turns `test_sync_denied_lock_raises_409_and_skips_sync` RED — no
lock is consulted, no 409 is raised, and `sync_from_qbo` runs unlocked.
"""
...
def test_sync_denied_lock_raises_409_and_skips_sync(monkeypatch):
    ...
    # U-347 §4 (/em Gate-2 approved): the 409 text drops the redundant
    # "for realm {id}" segment — the lock was always entity-only, not
    # realm-scoped, so the old wording was slightly misleading.
    assert exc_info.value.detail == "QBO account sync already in progress. Try again shortly."
