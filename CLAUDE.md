...
- **Contact entity**: Polymorphic child entity with nullable FKs to User, Company, Customer, Project, Vendor. Fields: Email, OfficePhone, MobilePhone, Fax, Notes. Uses instant workflow (ProcessEngine.execute_synchronous). API surface intact. **No UI as of 2026-04-24** — the Jinja inline-on-parent pattern (`shared/partials/contacts_*.html`) went dark when parent entity Jinja pages were purged in Waves B/C and no React replacement has been built yet. Partials still sit in `shared/partials/` but aren't included anywhere; reimplement in React when Contact management comes back.
