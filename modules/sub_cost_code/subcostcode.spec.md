...
- **API layer (`modules/sub_cost_code/api`)**
  - `router.py`: FastAPI `APIRouter` registered at `/api/v1` with tags `["api", "sub-cost-code"]`. Handlers usually create a new `SubCostCodeService()` per request, convert returned business objects via `.to_dict()`, and funnel all `DatabaseError` instances through `_handle_database_error` (409 on concurrency/duplicate text, 422 when the message mentions a missing parent cost code, otherwise 500). The detail route (`get/sub-cost-code/{public_id}`) invokes `SubCostCodeService.read_by_public_id` directly on the class, matching the current implementation even though the method expects an instance.
