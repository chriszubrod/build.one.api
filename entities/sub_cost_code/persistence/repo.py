        ...
        *,
        ...
        try:
            with get_connection() as conn:
                ...
                call_procedure(
                    cursor=cursor,
                    name="SetSubCostCodeQboIdentity",
                    params={
                        "Id": id,
                        "QboId": qbo_id,
                        "RealmId": realm_id,
                ...
                if row and getattr(row, "Stolen", False):
                    logger.warning(
                        "SubCostCode %s stole QBO identity (qbo_id=%s realm_id=%s) from a different "
                        "SubCostCode row — a stale duplicate identity existed before this stamp",
                        id, qbo_id, realm_id,
                    )
        except Exception as error:
