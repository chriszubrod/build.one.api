...
- [ ] **Wave E — shared-infra deletion (last step):** delete `get_current_user_web`, `require_module_web`, `WebAuthenticationRequired` + its `app.py` handler, CSRF helpers, `entities/auth/web/controller.py`, the 5 QBO `web/controller.py` files, and the `templates/` tree. Decide whether to ship **auth password reset** in the same wave or defer.
