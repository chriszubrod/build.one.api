...
IF OBJECT_ID('dbo.ExpenseFolderRunItem', 'U') IS NULL
BEGIN
CREATE TABLE [dbo].[ExpenseFolderRunItem]
(
    ...
    [Result] NVARCHAR(MAX) NULL,             -- JSON: { expense_public_id, skip_reason, ... }
    [StartedAt] DATETIME2(3) NULL,
    [CompletedAt] DATETIME2(3) NULL,
    -- Matches the live dbo.BillFolderRunItem shape (gap2 CreatedByUserId threading).
    [CreatedByUserId] BIGINT NOT NULL CONSTRAINT [DF_ExpenseFolderRunItem_CreatedByUserId] DEFAULT (17),
    CONSTRAINT [FK_ExpenseFolderRunItem_Run] FOREIGN KEY ([RunId]) REFERENCES [dbo].[ExpenseFolderRun]([Id])
);
END
GO

IF OBJECT_ID('dbo.ExpenseFolderRunItem', 'U') IS NOT NULL AND NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = 'FK_ExpenseFolderRunItem_CreatedByUser')
    ALTER TABLE [dbo].[ExpenseFolderRunItem] ADD CONSTRAINT [FK_ExpenseFolderRunItem_CreatedByUser] FOREIGN KEY ([CreatedByUserId]) REFERENCES [dbo].[User]([Id]);
GO

IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_ExpenseFolderRunItem_CreatedByUserId' AND object_id = OBJECT_ID('dbo.ExpenseFolderRunItem'))
    CREATE INDEX [IX_ExpenseFolderRunItem_CreatedByUserId] ON [dbo].[ExpenseFolderRunItem]([CreatedByUserId]);
GO

IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_ExpenseFolderRunItem_Status_ClaimedAt')
BEGIN
    CREATE INDEX IX_ExpenseFolderRunItem_Status_ClaimedAt ON [dbo].[ExpenseFolderRunItem]([Status], [ClaimedAt]);
END
GO

IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_ExpenseFolderRunItem_RunId_Status')
BEGIN
    CREATE INDEX IX_ExpenseFolderRunItem_RunId_Status ON [dbo].[ExpenseFolderRunItem]([RunId], [Status]);
END
GO


CREATE OR ALTER PROCEDURE CreateExpenseFolderRunItem
(
    @RunId BIGINT,
...
BEGIN
    ...
    INSERT INTO dbo.[ExpenseFolderRunItem]
...
CREATE OR ALTER PROCEDURE ClaimNextExpenseFolderRunItem
(
...
BEGIN
    ...
    ;WITH candidate AS (
        SELECT TOP 1 [Id]
        FROM dbo.[ExpenseFolderRunItem] WITH (UPDLOCK, READPAST, ROWLOCK)
    ...
    SET
        ...
        @ClaimedId = i.[Id]
    FROM dbo.[ExpenseFolderRunItem] i
...
CREATE OR ALTER PROCEDURE UpdateExpenseFolderRunItemOnSuccess
(
...
BEGIN
    ...
    UPDATE dbo.[ExpenseFolderRunItem]
    SET
        [Status] = @Status,
        [Result] = @Result,
        [LastError] = NULL,
...
CREATE OR ALTER PROCEDURE UpdateExpenseFolderRunItemOnFailure
(
    @PublicId UNIQUEIDENTIFIER,
...
CREATE OR ALTER PROCEDURE CheckAndCompleteExpenseFolderRun
(
    @RunId BIGINT
)
AS
...
CREATE OR ALTER PROCEDURE ReadExpenseFolderRunAggregateByPublicId
(
...
BEGIN
    ...
    FROM dbo.[ExpenseFolderRun] r
    LEFT JOIN dbo.[ExpenseFolderRunItem] i ON i.[RunId] = r.[Id]
...
CREATE OR ALTER PROCEDURE ReadExpenseFolderRunItemErrorsByRunPublicId
(
    @RunPublicId UNIQUEIDENTIFIER
)
AS
BEGIN
    ...
    SELECT
        ...
        i.[Attempts]
    FROM dbo.[ExpenseFolderRunItem] i
    INNER JOIN dbo.[ExpenseFolderRun] r ON i.[RunId] = r.[Id]
    WHERE r.[PublicId] = @RunPublicId
...
CREATE OR ALTER PROCEDURE ReadActiveExpenseFolderRunItemIds
(
    @RecentWindowMinutes INT = 60
)
AS
BEGIN
    -- Returns the SharePoint ItemId of every ExpenseFolderRunItem the
...
CREATE OR ALTER PROCEDURE AutoFailStaleExpenseFolderRuns
(
