...
- **QUIESCE BEFORE YOU DIAGNOSE — never draw a conclusion from a surface while async work against it is still in flight.** Every external surface in this workflow settles asynchronously: MS/Box outbox rows drain on their own timers, and the scheduler's QBO pulls rewrite `dbo` underneath you. A workbook read taken mid-drain shows half-applied state, and reporting it is reporting a snapshot of a moving system as if it were the finished one. Before reading a workbook (or `dbo`) to JUDGE it, prove it is settled:
  1. every outbox row for the entity is terminal (`done`; zero `pending`/`in_progress`/`failed`) — on BOTH `ms.Outbox` and `box.Outbox`;
  2. the file's own version/`modified_at` is **unchanged across two reads separated in time** (Box exposes `version_number`; SharePoint exposes `lastModifiedDateTime`);
  3. for a `dbo`-vs-QBO judgement, the relevant watermark has not advanced since your read.
  If any of those move, you are looking at work in progress — wait and re-read. (U-425, 2026-09-10: a Box DETAILS read taken while `update_box_excel` rows were still draining showed three charges appended at the sheet's bottom with blank/placeholder cost codes; it was reported to the operator as KI-36 corruption and a live-workbook repair was proposed. The drain then filed all three correctly into their cost-code sections. In the same window `dbo`'s cost code for a fourth charge changed from 40 to 39 as the scheduler pulled the operator's QBO edit — so a "dbo says 40, sheet says 39" mismatch resolved itself. Nothing was broken; the instrument was reading a system mid-settle.)
- **To attribute a CHANGE to an action, diff the artifact against its own PRIOR VERSION — never against a different system.** Comparing a live workbook to `dbo` tells you they disagree; it cannot tell you *who* made them disagree or *when*. Both SharePoint (Graph `/versions`, 200 kept) and Box (`/files/{id}/versions`, 50 kept) retain history — use it. (Same incident: six charges absent from the live sheet were reported as "dropped by the operator's workbook swap" on a live-sheet-vs-`dbo` comparison; a version diff showed they were absent from the pre-swap version too, so the swap had not touched them. The swap's real cost was ONE row.)
- **Compare like with like, and normalize numerics before calling a mismatch.** `ws.max_row` (openpyxl, includes trailing formatting) and the Graph used-range row count are different measures — mixing them manufactured a "1,435 rows lost" figure where the real non-empty delta was 15. Excel stores cost codes as NUMBERS: `08` reads back as `8` and `24.00` as `24`, so a string compare reports dozens of false mismatches (55 of them, in the same incident; the true count was 4). Coerce both sides to a common numeric form first.
- **A zero result must PROVE it can produce a non-zero one.** Any corpus-wide scan, filtered query or presence check that returns "none found" is only evidence if you also report the count it matched on at a coarser level — records scanned, and refs/rows of the right SHAPE seen anywhere. A query reading a field that does not exist returns an empty set exactly like a genuine absence, and "I searched everything and found nothing" is the most confident wrong answer this playbook can produce. Print the sanity count next to the zero, every time, and treat a scan with a zero coarse-count as BROKEN rather than clean. (U-425, 2026-09-09: a 21,360-record realm scan for a purchase's receipt read a nested `entity_ref` that the model does not have — the real fields are flat — so it reported "no document exists in QBO" for two charges whose PDFs the operator had open on screen. The corrected scan reports `Purchase refs seen anywhere = 4773` beside the hit count; a 0 there would have exposed the bug instantly.)
...
**Serialize QBO-heavy work INSIDE a single run — one realm-scale operation at a time.** The per-entity SQL applock on the sync scripts (`@qbo_sync_locked_cli("invoice")`, `"bill"`, `"purchase"`, `"vendorcredit"`) only serializes runs of the SAME entity. Two different entities take two different locks and proceed concurrently, and the attachable path takes **no lock at all**. Nothing in the codebase bounds you against the two things that actually constrain a run: the QBO API rate limit and the SQL tier. Do not start a second pull, ingest or realm-scale query while one is in flight — not in another shell, not in the background. (U-425, 2026-09-09: an attachable ingest run concurrently with a scoped vendor-credit pull produced a DB login timeout, four unexplained `qbo.http.request.failed` entries with no traceback, and a `Communication link failure` on a script that still exited **0**. Run alone, each finished cleanly.) Corollary: an exit code of 0 from a sync script is not success — verify the intended rows in the database afterward.
...
- **Fetch by identifier whenever you have one; the full-realm scan is for proving ABSENCE, not for lookups.** `query_all_attachables()` pages the entire realm (21,371 attachables as of 2026-09-09) and costs minutes; it is the right tool only when you must prove no document exists anywhere. When you already know the attachable id, `QboAttachableClient.get_attachable(id)` + `download_attachable(a)` answers in seconds. When you know the PARENT and want its documents, QBO **does** filter server-side — see the note below. (U-425, 2026-09-09: a comparison of two known attachables was run through the realm scan and took ~10 minutes; the same answer by direct fetch took 6 seconds.)
- **Server-side parent filter (verified against live QBO 2026-09-09 — supersedes KI-28's premise for lookups):** `SELECT * FROM Attachable WHERE AttachableRef.EntityRef.value = '<entity qbo id>'` returns exactly that entity's attachables. Proven against full-scan ground truth: multi-attachable parents returned every row, an attachable sitting past position 5,000 was found (the original KI-28 failure mode), and a nonsense id returned **0 rows** — so it is a real filter, not the misleading 200 the old per-entity query produced. It filters on the ref VALUE only, not the type, so keep the existing exact `(entity_ref_type, entity_ref_value)` check in memory — you are now applying it to 1-4 rows instead of 21,371. Retire-the-scan work is booked as U-433; until that ships, prefer this filter for lookups and keep the scan for absence proofs.
- For a definitive **absence** proof, use paginated `QboAttachableClient.query_all_attachables()` and inspect every relevant `attachable_ref`. **The ref's fields are FLAT — `ref.entity_ref_type` (e.g. `'Purchase'`, `'Bill'`, `'VendorCredit'`, `'Invoice'`) and `ref.entity_ref_value` (the QBO id as a string). There is NO nested `ref.entity_ref` object**; reading one yields `None` for every record and silently turns the whole scan into a guaranteed zero. Match on the exact type string AND the id, and print the count of refs of that type seen anywhere as your zero-result guard (Direct-call guardrails).

```python
matches = []
seen_of_type = 0
for a in client.query_all_attachables():
    for ref in (a.attachable_ref or []):
        if (ref.entity_ref_type or "") == "Purchase":
            seen_of_type += 1
            if str(ref.entity_ref_value) == str(purchase_qbo_id):
                matches.append((a.id, a.file_name, a.content_type))
print(f"matches={len(matches)}  Purchase-refs seen anywhere={seen_of_type}")  # 0 on the right = BROKEN, not clean
``` A receipt's attachment location is evidence to inspect, not proof of its billing source.
...
**2c. Source gap-fill. Confirm each source EXISTS LIVE before calling it missing — and never conclude from the watermark that it arrived.** Shared invariant 4 applies to sources, not just the invoice: a current `vendorcredit` / `bill` / `purchase` watermark says a tick ran, not that this particular transaction was projected. Resolve the source in **live QBO first** (`QboVendorCreditClient.query_all_vendor_credits(start_date=...)`, `QboPurchaseClient.query_all_purchases(start_date=...)`, or `get_*` by id when you have it), then check for its local row by `QboId` + `RealmId`. Live-present + locally-absent is a **projection gap**, not a missing source, and it is invisible to every local-only check including the source-link proposal — which reports `no_match` for it exactly as it would for a charge with no source at all. (U-425, 2026-09-09: a `-$119.84` credit line on SHT-25 proposed as `no_match`; live QBO held VendorCredit `75423` correctly coded, and the `vendorcredit` watermark was 4 minutes old. It had simply never been projected into `dbo.BillCredit`, so the matcher had nothing to match against.)
...
**3b. Source onboarding and documents.**

⚠ **Before treating a source line as undocumented, compare the parent's LOCAL lines against its LIVE lines — the document may be attached to a line QBO no longer has.** A local source parent can retain a line whose QBO line id was replaced when the operator re-entered the charge; the ingested receipt stays linked to that dead line while the live line shows zero attachments and the coverage check halts on a charge that is, in fact, fully documented. Read the live parent (`QboPurchaseClient.get_purchase_raw(id)` / `get_bill` / `get_vendor_credit`), collect its line ids, and flag any local line whose `QboId` is absent from that set as an ORPHAN. Two tells that you are looking at one rather than a genuinely missing document: the parent's local lines do not sum to its header, and a sibling line carries the attachment. The repair is to link the existing attachment to the LIVE line (that line has no link, so `create(...)` is the right call — see CRITICAL #5) and to reconcile the orphan separately; it is NOT an ingest. (U-425, 2026-09-09: SHT-25's `$1,050.35` Dumpster charge — live Purchase 75568 had one line, id 2; locally an extra line id 1 survived holding the Ramp receipt, so the live line read as undocumented and lines summed to `$2,100.70` against a `$1,050.35` header.)

Read sources by their native QBO identity, then inspect the actual child rows. For example:
...
49. **KI-49 — A scan that reads a field the model does not have returns "none found", indistinguishable from a genuine absence** (SHT-25, 2026-09-09, U-425). A realm-wide receipt hunt iterated all 21,360 attachables and reported **zero** documents for two charges whose PDFs the operator had open in QBO. Cause: it read a nested `ref.entity_ref`; `QboAttachableRef` exposes **flat** `entity_ref_type` / `entity_ref_value` and no nested object, so every comparison silently evaluated against `None`. The scan could not have returned a hit for any input. **Rule: never report a zero without the coarse count beside it** (records scanned AND refs/rows of the right shape seen anywhere). A zero coarse-count means the instrument is broken, not that the thing is absent. This applies to every presence check in this playbook — attachables, duplicate screens, cross-project screens, the double-bill screen.
50. **KI-50 — The document can be attached to a source line QBO no longer has** (SHT-25, 2026-09-09, U-425). A local source parent can keep a line whose QBO line id was replaced when the operator re-entered the charge. The ingested receipt stays bound to that dead line; the live line reads as undocumented and the Step 5 coverage check halts on a fully documented charge. Tells: the parent's local lines do not sum to its header, and a sibling line holds the attachment. Diagnose by reading the LIVE parent's line ids and flagging any local line whose `QboId` is not among them (Step 3b). The repair is a re-link onto the live line plus separate reconciliation of the orphan — **not** an ingest, and not a halt.
51. **KI-51 — Reading a surface mid-settle produces confident, wrong findings** (SHT-25, 2026-09-10, U-425). Three separate false alarms in one session, all the same shape: a measurement taken while the system was still converging, reported as a defect. (a) A Box DETAILS read during an `update_box_excel` drain showed three SHT-25 charges bottom-appended with blank/`00.02` cost codes — the KI-36 danger signature — and a live-workbook repair was proposed to the operator; the drain then filed all three correctly (rows 528/1280/2282). (b) `dbo`'s cost code for a fourth charge read 40 while the sheet read 39 and was reported as drift; minutes later the scheduler's pull landed the operator's QBO move and `dbo` became 39 — the sheet had been right all along. (c) Six charges missing from the live sheet were attributed to the operator's Box→SharePoint workbook swap; a diff against SharePoint version 583 showed they were missing BEFORE the swap too. **The swap's actual cost was one untagged $715 row.** Quiesce first (Direct-call guardrails), diff against the artifact's own prior version, and normalize numerics — `08` vs `8` and `24.00` vs `24` inflated a 4-row difference into 55.
