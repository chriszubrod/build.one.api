        ...
        try:
            with get_connection() as conn:
                ...
                if row and getattr(row, "Stolen", False):
                    logger.warning(
                        "Customer %s stole QBO identity (qbo_id=%s realm_id=%s) from a different "
                        "Customer row — a stale duplicate identity existed before this stamp",
                        id, qbo_id, realm_id,
                    )
        except Exception as error:
            logger.error(f"Error during set customer qbo identity: {error}")
            raise map_database_error(error)
