...
> **Last verified against code:** 2026-07-13 (full claim-by-claim audit; every service/connector/SQL reference below was checked against the codebase on that date). Partial re-verification 2026-08-07 (TB3-20): CRITICAL #8's writer-dedup path (`BillService.sync_to_excel_workbook`) and `entities/invoice/business/naming.py` were re-read against code; the rest of the file still carries the 2026-07-13 date. When editing this file, re-verify the code references you touch and update this line.
...
- **The invoice must be a linked, coded draw.** G702/G703 + the current draw's own Draw Request rollup render only when the invoice appears in `DrawFinancialsService.coded_draws_for_project` — i.e. it has ≥1 **non-Manual** line (cost-coded + source-FK'd via **Step 4** linkage). An invoice whose lines are ALL `SourceType='Manual'` (freshly QBO-pulled, un-linked) has an empty cost-code rollup → **no G702/G703, an empty Draw Request, and `_generate_invoice_packet` raises 400 "No PDF attachments found"** (no non-Manual line → no attachment). **Step 4 linkage is a hard gate for a meaningful packet, not just for reconciliation** (e.g. HA-05/proj-128 sat as 52 Manual lines, $0 coded → not packet-ready until linked).
- **The Trend spans EVERY historical pay application (U-271), not just coded draws.** Its columns come from `DrawFinancialsService.all_draws_for_project` (a superset of `coded_draws_for_project`): coded draws use the local source-linked rollup; **early/migrated draws** — all-`Manual` locally but carrying the cost-code Item hierarchy in `qbo.InvoiceLine.ItemRefName` (`"CostCode:SubCostCode"`) — are rolled up from QBO, so a project's whole draw history shows as one column-per-draw matrix. The `qbo.ItemSubCostCode` mapping table is **unreliable** (maps '02 Dumpsters'→CostCode 09) — resolution parses the self-describing `ItemRefName` string, and any line with no cost-code Item (e.g. a split `"5% markup"` line) lands in an **"Uncoded"** row so each column still foots to its invoice total. A QBO-pull mirror of a coded draw (same date + billed total, e.g. `MR2-MAIN-05-2`) is dropped; same-draw re-issues (`MR2-MAIN-04` + `-04-2`) merge into one column. **G702/G703 deliberately still consume `coded_draws_for_project`** (they reconcile to the Budget SoV, a coded-only surface).
...
- **Builder's Fee + G702 header come from the project's `dbo.Contract`.** Fee line = subtotal × `Contract.BuildersFeeRate`; the G702 Owner/Architect/Contract-For/Contract-Date fields come from the Contract header. **Until a `dbo.Contract` row with the fee rate is persisted for the project, the rate-derived fee renders $0.** Fee-rate half is now stored for **proj-128** (`dbo.Contract` `BuildersFeeRate=0.14`, 2026-08-06). The **G702 header fields** (Owner/Architect/Contract-For/Date) are NOT yet `dbo.Contract` columns — a schema unit is queued; until then the router renders Owner from the Customer and the rest blank, so pass them explicitly at generation if the review packet needs them (as HA-04/HA-05 did).

**Cost-only vs fee-inclusive draws — get the Builder's Fee into QBO BEFORE completing (HA-05, 2026-08-06).** A draw's QBO invoice may be built at **cost** (no Builder's Fee line — HA-01/02/03/05) or **fee-inclusive** (an explicit Builder's Fee line — HA-04). Detect it: if `dbo.Invoice.TotalAmount == Σ(non-Manual coded line amounts)` the draw is cost-only; if `>`, the difference is the fee (carried as a **Manual** line, excluded from the coded rollup). **The packet's two fee sources disagree on a cost-only invoice:** the **Draw Request** fee is *invoice-derived* (`_draw_fee_from_invoice` = `invoice.total − coded subtotal`), while **G702/G703/Trend** are *rate-derived* (`coded_draws_for_project` = subtotal × `Contract.BuildersFeeRate`). They match only when the invoice already carries the fee (fee == subtotal × rate). On a cost-only invoice the regenerated Draw Request shows a **$0 fee** while the other pages show the rate fee → an inconsistent, non-reconciling packet. **Sequence for a draw that should carry a fee:** (1) the user adds the Builder's Fee line to the **QBO invoice first** (so `qbo.Invoice.TotalAmt = cost × (1+rate)`); (2) re-pull — the scheduler's invoice sync (or a manual pull) brings it in as a Manual "Builder's Fee" line (verify: no phantom orphans, prior linked FKs intact, `dbo` total == qbo); (3) THEN `complete_invoice` — the regenerated packet is consistent and reconciles. HA-05: cost-only $305,770.42 → user added a $42,807.86 (14%) fee line in QBO → re-pull → complete → packet **$348,578.28 across every page**. **Never `complete_invoice` a cost-only draw that should carry a fee without adding the QBO fee line first** — you would ship a $0-fee Draw Request. (The "unify fee sourcing" TODO — make the Draw Request read the Contract rate — would decouple this; until then the QBO-fee-first sequence is mandatory.)
...
**Phase 1 — read-only audit (no external writes, no user interaction).** Run Steps 1 (incl. 1b/1c), 2, and 3 (diagnose 3a–3d needs; do NOT execute recoveries yet), then Step 4 as a **dry-run** (run the fingerprint SELECTs, record proposed matches, apply NO UPDATEs), then the Step 5 coverage query against the *proposed* matches, the KI-16 Price-NULL check, the KI-27 multi-line-Expense check, the KI-40 attachment page-audit (flag every linked attachment with >1 page for content spot-check), and the KI-41 duplicate-pair screen (equal/similar amounts + overlapping periods across different vendor records, same-type AND cross-type; compare vendor invoice numbers read out of the attachments — note QBO-attachable/receipt docs carry NO stored OCR (KI-42), so download + parse the blobs ad hoc). Collect every gap into one report:
...
| Same-transaction pair on the invoice — cross-type (KI-38) or same-type across different vendor records (KI-41, e.g. a WillScot/Mobile Mini duplicate-vendor split); same/similar amount, overlapping period, vendor names may differ | Double-bill check: user confirms which line stays; duplicate-vendor identity flagged for merge/alias decision |
...
**Pre-delete checklist (KI-43, OHR2-CHAPEL-09) — verify prerequisites IMMEDIATELY before the delete, not just in Phase 1:** (1) the project's `qbo.CustomerProject` mapping still exists — the connector re-run in part 3 requires it, and a scheduler customer-sync tick can drop it mid-run; (2) the `qbo.Invoice` row + full `qbo.InvoiceLine` set are current. Only after both check out, delete. If the connector still raises after the delete, the invoice does not exist anywhere local — treat the window as an open incident: re-heal the mapping (Step 1 heal; also check for a freshly-minted duplicate `dbo.Project` per KI-43), re-run the connector, and re-verify Step 3 before doing anything else.
...
**Hand-entered ReimburseCharges — the source bill exists but is NOT linked to the RC (HA-05, 2026-08-06).** Not every RC is auto-generated from a billable Bill/Purchase line. A user can **type a billable charge directly onto the QBO invoice**: query it live (`SELECT * FROM ReimburseCharge WHERE Id IN (...)`) and it shows `Line[].DetailType='ReimburseLineDetail'` with an `ItemRef` to a service item and `ItemAccountRef` to an **income** account (e.g. "Construction Income"), a `CreateTime` ≈ the invoice-build date, and **no back-LinkedTxn to any Bill/Purchase** (only forward to the Invoice). So fingerprint (4.1), the rc_map hop, AND the invoice's AttachableRef map all find nothing for that line — it looks like a source-less Manual line with no document. **But the real vendor bill usually DOES exist in QBO, just un-linked to the RC — do not accept it as source-less.** Ask the user for the vendor + the bill's DocNumber (they read it off the QBO line / their records), then confirm live: `SELECT * FROM Bill WHERE DocNumber IN (...)` matching amount + TxnDate + CustomerRef, and verify its attachable (`query_all_attachables` filtered to the bill's id). If it's not yet in local staging/dbo, scoped-pull it **with attachments** (Step 2c/3) and link normally (Step 4). HA-05: L46 "Siding Materials" $4,531.13 (hand-entered RC 74218, no source) → Walker Bill **#240040** at the same amount/date with `IN205AAD.pdf`; L51 "Marmiro Stones BALANCE" $20,592.97 (RC 74207) → Marmiro Bill **#MR-52799-02** with `MR-52799.pdf`. Both onboarded + linked → 52/52. Only if no such bill exists in QBO is it a genuine manual line (classify per CRITICAL #5).
...
6. **Line REMOVALS don't propagate** (HA-04): removing a line in QBO deletes the `qbo.InvoiceLineItemInvoiceLine` mapping but strands the dbo ILI — detect via the Step 4 phantom-orphan query, delete via `InvoiceLineItemService().delete_by_public_id(...)`, un-bill its source (`_reset_source_as_unbilled`), and confirm `dbo.Invoice.TotalAmount` matches the new QBO total. **Then propagate to every external surface — none of it is automatic (OHR2-CHAPEL-09):** clear the removed line's column-H DRAW stamp in the SharePoint DETAILS **and** in the Box DETAILS (they don't cross-propagate — KI-39; fixing one leaves the other stale). Box has no row-delete/cell primitive, so its side is a manual openpyxl fill-in-place edit under the production lock lifecycle (never `insert_rows`/delete rows — KI-36 discipline). Delete the removed line's orphan PDF from the SharePoint draw subfolder AND the Box `15 - Draw Requests/<inv>/` subfolder — but only when no surviving line shares that attachment. Regenerate the packet, then re-run the full Step 10 matrix. A first-class removal path that does all five surfaces in one call is a TODO.
...
16. **KI-16 (formerly #17) — [FIXED 2026-07-03, verify deployed] Column N Price→Amount fallback** now exists in the single-bill `BillService.sync_to_excel_workbook` AND the Box row builder (parity), matching `sync_bills_batch_to_excel`. The 7a Price pre-flight remains a cheap belt-and-suspenders (`Price = Amount` also keeps dbo self-consistent); on a pre-fix deployment it is mandatory (HP2-09 / Q44862, 2026-05-15: NULL Price → $0 row). **A $0 row already written to the Box workbook stays frozen at $0 even after the Price fix** — col-Z idempotency skips it on re-sync and the invoice path stamps only column H; see KI-46 for detection + surgical fill (OHR2-36: $18,630).
...
41. **KI-41 — Duplicate-vendor records double-bill the same transaction** (OHR2-CHAPEL-09, 2026-07-08: "WillScot" and "Mobile Mini" are the SAME merged company held as two vendor records — qbo 380/1305, dbo 1055/631; the same two December invoices 9025307153/9025307159 were booked twice, once as WillScot card charges WITH DocNumbers and again a day later as Mobile Mini charges WITHOUT — both landed on the draw: $925.54 double-bill caught by the operator on the finished packet). Every existing guard missed it: KI-38 keys on cross-type Bill-vs-Expense pairs (these were Expense-vs-Expense), the Step 4 fingerprint keys on amount+description+date+customer (dates differed by a day, vendor differed), and byte-hash dedup passed (different scans of the same invoice). **Phase-1 screen (extends KI-38):** flag every pair of lines — same-type OR cross-type, across DIFFERENT vendor records — with equal/similar amounts and overlapping service periods; where attachment text is readable, compare the **vendor invoice number extracted from each linked attachment** across ALL lines on the invoice (the only signal that catches different-scan duplicates). Flag duplicate-vendor identities (two vendor records that alias one company) for a user merge/alias decision — automated detector + vendor-merge workflow tracked in TODO.
42. **KI-42 — Card-charge Purchases without a QBO DocNumber carry a placeholder ReferenceNumber, and manual fixes revert** (OHR2-CHAPEL-09). The purchase connector sets `reference_number = doc_number or "QBO-<qbo_id>"` and **re-passes it unconditionally on every re-pull** (`integrations/intuit/qbo/purchase/connector/expense/business/service.py` sync/update path) — so a hand edit to `dbo.Expense.ReferenceNumber` is clobbered by the next scheduler purchase-sync. The real vendor invoice number usually exists ONLY inside the attachment PDF (no stored OCR — the DI extraction pipeline runs only on email-intake, so `dbo.Attachment` extraction fields are null on QBO-attachable/receipt docs; content checks must download + parse blobs ad hoc). Consequences: packet TOC and DETAILS "INVOICE #" show `QBO-#####` placeholders. **Durable interim: put the real number in the QBO Purchase's DocNumber (Ref no.) field** — QBO owns that field, the next pull propagates it, and it sticks. Never rely on a dbo-side edit. Durable fix (extraction at intake + a sync-proof field) tracked in TODO.
43. **KI-43 — The project's `qbo.CustomerProject` mapping can VANISH mid-run** (OHR2-CHAPEL-09: present in Phase 1, gone by the correction session — the Step 3a connector re-run then failed AFTER the dbo.Invoice was already deleted, leaving a no-invoice window). The only code path that deletes these mappings is the customer-sync connector (`integrations/intuit/qbo/customer/connector/project/business/service.py` ~line 110): when a mapping exists but the Project read returns nothing it DELETES the mapping and re-binds/creates by name — so a scheduler customer-sync tick can drop and re-shape mappings underneath a session (root-cause investigation in TODO). Consequences for a run: (a) **re-verify the mapping IMMEDIATELY before any destructive reset** (Step 3a pre-delete checklist), not just in Phase 1; (b) when a mapping vanishes, also check for a freshly-minted duplicate `dbo.Project` (that delete path's recreate side) before re-healing per Step 1.
44. **KI-44 — The prod scheduler's QBO pulls race interactive sessions** (OHR2-CHAPEL-09: mid-correction, a purchase re-sync shifted expense dates 12/31→01/01 and threatened to revert ReferenceNumber edits). There is NO pause/coordination primitive today (tracked in TODO) — and do NOT try to pause timers via `AzureWebJobs.<fn>.Disabled` (Flex Consumption de-registers the function; recovery requires a full redeploy). Discipline instead: treat every QBO-owned field (dates, amounts, DocNumber/ReferenceNumber, line sets) as volatile — re-read it immediately before depending on it in a write or packet; make local edits only to fields QBO does NOT own (KI-42 lists the trap); keep destructive-reset windows as short as possible (Step 3a ordering) so a mid-window sync tick has nothing to race.
45. **KI-45 — [FIXED 2026-07-13, verify deployed] SharePoint rejects line-PDF uploads whose decoded URL path exceeds 400 chars** (OHR2-36: 16 contract-labor PDFs failed in `_upload_to_sharepoint` — multi-sentence CL narratives made ~330-char filenames; packet + short-named files fine; Box unaffected). `build_line_pdf_filename` now clips the description to 120 chars and hard-caps the base name at 200 (`entities/invoice/business/naming.py`) — the cap lives in the shared helper so SharePoint and Box names stay identical. On a pre-fix deployment: expect long-named CL files to fail Step 9 with a URL-length error; recover by re-uploading with a clipped description. **Legacy skew note:** files uploaded before the fix (or via the OHR2-36 manual recovery) can have full-length Box names vs truncated SP names — cosmetic, don't chase.
...
47. **KI-47 — A control character in a line description aborts the ENTIRE SharePoint upload batch** (TB3-20, 2026-08-07). `BillLineItem.Description` can carry embedded newlines — a worker's multi-line note arrives verbatim from QBO. `build_line_pdf_filename` sanitized reserved characters and capped length, but an interior `\n` survived `.strip()` + the 120-char clip, landed in the filename, and then in the Graph URL path, where httpx raises `InvalidURL("Invalid non-printable ASCII character in URL")`. Because that exception escapes the per-file loop in `_upload_to_sharepoint`, **one bad description took down all 133 files** (`synced_count: 0`, `success: false`) — not just its own. Sibling of KI-45: that capped filename LENGTH, this fixes filename LEGALITY. **Fixed 2026-08-07** (`01426e5`) by adding `\x00-\x1f\x7f` to `_FILENAME_SANITIZE_RE` in `entities/invoice/business/naming.py` — in the shared helper so SP and Box names stay aligned, replacing with `_` not a space (the Box outbox collapses whitespace runs, so a space would desync the two names). Names with no control characters are byte-identical, so no legacy skew. On a pre-fix deployment the symptom is a whole-batch Step 9 failure with a URL error naming a character position; find the culprit with `Description LIKE '%'+CHAR(10)+'%' OR ... CHAR(13) ... OR ... CHAR(9)` over the invoice's source lines.
