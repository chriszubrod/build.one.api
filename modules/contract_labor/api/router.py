...
from modules.contract_labor.business.import_service import ContractLaborImportService
from modules.contract_labor.api.schemas import (
    ContractLaborCreate,
    ContractLaborUpdate,
...
def create_contract_labor(contract_labor: ContractLaborCreate):
    """
    ...
    try:
        ...
        logger.exception("Error creating contract labor")
...
def update_contract_labor(public_id: str, contract_labor: ContractLaborUpdate):
    """
    Update a contract labor entry by public ID.
    """
    try:
        service = ContractLaborService()
        result = service.update_by_public_id(public_id=public_id, contract_labor=contract_labor)
        if not result:
            raise HTTPException(status_code=404, detail="Contract labor entry not found")
        return ContractLaborResponse(
            id=result.id,
        ...
        logger.exception("Error updating contract labor")
        raise HTTPException(status_code=500, detail=str(e))


@router.delete("/{public_id}", response_model=ContractLaborResponse)
def delete_contract_labor(public_id: str):
    """
    Delete a contract labor entry by public ID.
    """
    try:
        service = ContractLaborService()
        result = service.delete_by_public_id(public_id=public_id)
        if not result:
            raise HTTPException(status_code=404, detail="Contract labor entry not found")
        return ContractLaborResponse(
            id=result.id,
        ...
        logger.exception("Error deleting contract labor")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_billing_summary():
    ...
    """
    try:
        from modules.contract_labor.business.bill_service import ContractLaborBillService
...
async def generate_pdfs_for_bill(bill_public_id: str):
    ...
    """
    try:
        from modules.contract_labor.business.pdf_service import ContractLaborPDFService
