...
IF OBJECT_ID('dbo.EmailMessage', 'U') IS NOT NULL
   AND NOT EXISTS (SELECT 1 FROM sys.columns
                   WHERE object_id = OBJECT_ID('dbo.EmailMessage') AND name = 'CreatedByUserId')
BEGIN
    ALTER TABLE [dbo].[EmailMessage] ADD [CreatedByUserId] BIGINT NOT NULL
        CONSTRAINT [DF_EmailMessage_CreatedByUserId] DEFAULT (17);
END
GO
IF OBJECT_ID('dbo.EmailMessage', 'U') IS NOT NULL
   AND NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = 'FK_EmailMessage_CreatedByUser')
BEGIN
    ALTER TABLE [dbo].[EmailMessage] ADD CONSTRAINT [FK_EmailMessage_CreatedByUser]
