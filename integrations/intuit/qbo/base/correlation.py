...
_correlation_id_var: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "qbo_correlation_id", default=None
)

# Idempotency key propagation for outbox-driven writes. When the outbox worker
# is dispatching a retryable operation, it sets this context to the row's
# stable RequestId. QboHttpClient falls back to this value when no explicit
# idempotency_key is passed to post()/put(), which lets the worker drive
# idempotent writes without threading the key through every connector method.
_idempotency_key_var: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "qbo_idempotency_key", default=None
)
...
def get_idempotency_key() -> Optional[str]:
    ...
    Used by `QboHttpClient` as a fallback when no explicit key is supplied.
    """
...
def idempotency_key_context(key: str) -> Iterator[None]:
    ...
    Every POST/PUT made through `QboHttpClient` within this block will use
    `key` as the `?requestid=` value unless the caller explicitly passes a
    different one. On exit the previous context value (usually None) is
    restored.

    Typical use by the outbox worker:

        with idempotency_key_context(row.request_id):
            connector.sync_to_qbo_bill(bill, realm_id=row.realm_id)

    Retries of the same outbox row reuse the same key, which lets QBO
