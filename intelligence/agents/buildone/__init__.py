"""Build.One agent package — central orchestrator that delegates to specialists.

Importing this package triggers registration of:
  1. Sub-agents (whose own __init__ files bring in their entity tools).
  2. The delegation tool(s) Build.One uses to dispatch to those sub-agents.
  3. Build.One itself.

Build.One has no direct entity tools; every entity operation flows through a
delegation. Specialists own per-entity RBAC; Build.One's role only needs to
log in.

Callers that want Build.One available should `import intelligence.agents.buildone`
...
# (and update buildone.tools in definition.py) per new specialist.
...
# Register the Build.One agent.
from intelligence.agents.buildone.definition import buildone  # noqa: F401
