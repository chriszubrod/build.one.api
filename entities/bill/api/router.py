...
def _bill_dict_with_lifecycle(bill, *, review=None) -> dict:
...
async def get_bills_router(
    ...
    # U-444 made `submitted` flag-derived, so the review row carries everything
    # the kind needs — no per-page boundary lookup, no extra round-trip.
    bill_dicts = [
        _bill_dict_with_lifecycle(bill, review=review_map.get(bill.id))
        for bill in bills
    ]
...
async def get_bill_by_bill_number_and_vendor_router(bill_number: str, vendor_public_id: str, current_user: dict = Depends(require_module_api(Modules.BILLS))):
    ...
    def _fetch():
        ...
        if not bill:
            return None
        return bill, _current_review_for_bill(bill.id)

    result = await asyncio.to_thread(_fetch)
    if not result:
        raise_not_found("Bill")
    bill, review = result
    return item_response(_bill_dict_with_lifecycle(bill, review=review))
...
async def get_bill_by_public_id_router(public_id: str, current_user: dict = Depends(require_module_api(Modules.BILLS))):
    ...
    def _fetch():
        ...
        return bill, qbo_bill_url, vendor_public_id, _current_review_for_bill(bill.id)

    result = await asyncio.to_thread(_fetch)
    if not result:
        raise_not_found("Bill")
    bill, qbo_bill_url, vendor_public_id, review = result
    payload = _bill_dict_with_lifecycle(bill, review=review)
...
def get_bill_by_id_router(id: int, current_user: dict = Depends(require_module_api(Modules.BILLS))):
    ...
    if not bill:
        raise_not_found("Bill")
    payload = _bill_dict_with_lifecycle(bill, review=_current_review_for_bill(bill.id))
...
def _run_complete_bill(public_id: str, job_public_id: str | None = None, force: bool = False) -> None:
    ...
    try:
        bill = BillService().read_by_public_id(public_id=public_id)
        if not bill:
            # U-434: a job pointing at a Bill that no longer exists is an
            # anomaly, not a success. Marking success here retired the job and
            # hid it; mark_failure lets the watchdog re-check and then
            # dead-letter it at max_attempts (5) so it surfaces instead.
            logger.warning("Complete bill: no such bill, marking job failed: public_id=%s", public_id)
            if job_public_id:
                job_service.mark_failure(job_public_id, "Bill not found")
            return
        if not force and not getattr(bill, "is_draft", True):
            # Genuinely already done — an idempotent no-op, so success is correct.
            logger.info("Complete bill skipped (already completed): public_id=%s", public_id)
            if job_public_id:
        ...
        if job_public_id:
            # U-434 — the marking contract, corrected.
            #
            # This previously called mark_success() for ANY returned dict, on the
            # stated theory that "a returned dict = finalize+enqueue ran; outbox
            # retries external writes". That is false for complete_bill's early
            # returns (404 missing, 400 vendor missing, 500 finalize error): they
            # return BEFORE the finalize and before Step 5's outbox enqueue, so
            # nothing was queued and nothing retries. Marking them successful
            # retired the job, and claim_next_stuck keys on job status — so the
            # reclaim watchdog skipped them forever. The bill stayed IsDraft=1
            # and its AP silently never reached QBO/SharePoint/Excel/Box, while
            # the client had already been handed a 202.
            #
            # `bill_finalized` is the correct discriminator: it is True on every
...
# U-478: removed dead pending/single/prepare/move routes and the in-process
# single-file helper. Live: enqueue → /admin/bill-folder/tick → process_single_item.
# Do not re-add: -single NameError'd; -move let can_read callers delete PDFs.
