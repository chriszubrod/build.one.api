...
class _CreateContractLaborArgs(BaseModel):
    ...
    description: Optional[str] = Field(
        ...
        description=(
            ...
            "review. PM email approval flips to `ready` only when every "
            "project-anchored line on that record is coded (`ready_flipped` "
            "on apply_reviewer_decision)."
        ),
    )
...
find_contract_labor_by_conversation_id = Tool(
    ...
    description=(
        ...
        "{YYYY-MM-DD}`).\n\n"
        "CONSOLIDATION (U-210): the outbound notification is now ONE "
...
apply_contract_labor_reviewer_decision = Tool(
    ...
    description=(
        ...
        "CONSOLIDATION (U-210): CL review notifications are one "
        ...
        "  - On approval, ContractLabor.Status flips to `ready` ONLY when "
        "every project-anchored line item on that laborer's record has a "
        "SubCostCode — a worker split across projects stays `submitted` "
        "until each project's PM codes their lines (Generate Bills picks "
        "up only `ready` rows). On rejection, Status is untouched.\n\n"
        "**Return:** an aggregate (`crew_size`, `applied_count`, "
        "`ready_flipped_count`, `applied` per-CL results with "
        "`ready_flipped` per member, `failures`). A partial success "
